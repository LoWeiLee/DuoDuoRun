...
★★ **R60 結案，而且是在最要緊的區域結案。** `nortest::lillie.test` 走**第三套 dispatch**
（DW ＋ Stephens，與移植來源 statsmodels 無關），11 組探針的 $D$ **逐位元相符**；$p$ 分兩段：
**決策區（$p<0.1$，含五組 clampzone＝舊實作印 $p=1.000$ 的那五個點）max 絕對差 1.5e−8**
——連 `likert_n1000` 的 **2.8953e−114** 都對上；$p>0.1$ 的區域 max 8.9e−3，是兩套 dispatch 的慣例差異。
⇒ **不是「大致相符」，是兩套獨立編寫的實作在 DW 分支上逐值相同。**

★ **五項銷帳**：`descriptive` 的偏態峰度＝**e1071 type 2（SPSS/SAS 預設）逐位元相符**
（一句從未核實的檔頭斷言結案；三種算法的峰度差 1.7 倍）；Levene 兩個 center 慣例都對上；
Pearson、三支迴歸、`car::vif` 全對；★★ **階層迴歸的手算 $\Delta F$／$\Delta p$ 對上 R 的 `anova()`**
⇒ **§0 點名的「手算基準與 JS 出自同一次理解」結構性弱點在本組結案**；
標準化係數 beta（**基準裡根本沒有**）與 R 相符到全部印出位數 ⇒ **E80** 建議補進基準。

★ **R64（待裁決）——Spearman 的慣例分歧已量化，但結論與 R58 相反**：
本工具走 $t$ 近似（同 scipy），R 預設無並列時走精確法。216 組確定性排列
（$n$=6~60、$\rho$ 掃過全域）：本工具＝R 的 `exact=FALSE` 分支（max 相對差 3.8e−08）；
近似/精確比值 **0.288（$n$=6，$p$ 小 3.5 倍）~ 1.005**，方向系統性偏寬鬆但隨 $n$ 迅速收斂；
★★ **.05 判定翻面 0 組**（R58 當時量到 1.1% 偽顯著）⇒ ✅ **Kevin 核定：書面化即可，不改實作、不加 UI 警告**。
⚠️ ★★ **但前提是「全部無並列」——而 R58 正是在有並列時翻船的**，
且 Spearman 有並列時 R 的 `cor.test` 會退回近似 ⇒ **R 不能當那一格的權威**。
✅ **Kevin 核定：誠實標註未量化、列 backlog**，不在本批自行枚舉。
