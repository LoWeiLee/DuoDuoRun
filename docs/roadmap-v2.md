...
## ★ 下一個 session 從這裡開始（★ 專案已休眠，2026-07-30）

**一句話**：★★ **階段 A 已完成並結案**——64 份方法文件、`reference.json` 86 組基準
**100% 被文件第 6 節點名**、§6.7 的**七條判準全部達成**、§8 休眠與復工已填完。
專案自 2026-07-30 起休眠，**階段 B 最早 2026 年 9 月底啟動**。

### ⚠️ 復工的第一件事：讀 §8，不要直接開工

接手者對本輪對話**零記憶**，而 §8 就是為此寫的：

| 節 | 內容 |
|---|---|
| §8.1 | 復工第一件事的順序 |
| §8.2 | 環境重建（沙盒每次都是乾淨的） |
| §8.3 | **基準線驗收**——確認休眠期間沒壞，含每一項的預期值 |
| §8.4 | **休眠當下的狀態快照**——含三個 L4 的共同結構、刻意不處理的六項及其理由 |
| §8.5 | 給復工執行者的**四句話** |

★ **§8.4 明確寫了：復工後的第一批工作不是 Wave F1**，而是把階段 A 累積的約 60 項 E 編號分流——
其中有一批是「R 已經驗過、只差寫進 `generate_reference.py`」的，成本近乎零而風險很高。

---

### ★ 階段 A 留下的四條，比任何進度數字都重要

**1. 「有第三方對照」保護的是那一個基準點，不是整個參數空間。**
三個 L4 裡有兩個（R50、R60）落在 **tier A**——它們都有第三方逐值對照，
而失效區在**參數空間的另一個方向**（df、$n$）。
★ 三個 L4 的共同結構是：**唯一的基準點恰好落在安全區**。

**2. 逐值比對永遠比不到不存在的欄位。**
`compare.test.js` 是「欄位對欄位」的防線，覆蓋率上限就是基準組的欄位清單。
A6b 一批出現三個這一型（R72／R73／R74）——加權 kappa 的 CI 上界算出 **1.0248** 而三道防線全綠，
只因為 `cohen_kappa` 那一組根本沒有 CI 欄位。

**3. 「零變異／退化值」連中五次，分屬五個模組。**
A4 的 R40-i、A5a 的 R51、A6a 的 R61 與 R66、A6b 的 R72——
`fmtNum(Infinity)` 印「—」、`fmtP(0)` 印「< .001」、`toneForP(0)` 給綠燈，
**三個獨立的格式化決定疊起來，就把數學上無定義的結果渲染成一個看起來很確定的結論**。
★ 而 R69 又多一層：**格式化函式只擋 `Infinity` 與 `NaN`，擋不住「有限但荒謬」的值**
（多元迴歸完美配適時把 $t=$ `2312738254615615.50` 原樣印出來）。
⇒ **每支方法開工先跑一次常數欄與完美配適，三十秒。**

**4. SKIP 比 TOL 危險。**
A5b 與 A6a 各掃了一次容差表，多數放寬都是遺留的假放寬，
而**兩批的真缺口都在 SKIP**（`mann_whitney_small.pExact`、`ks_lilliefors.p`，後者是 L4）。
⇒ TOL 是「知道有差、量過」；SKIP 是「知道有差、沒量」。
...
| 項目 | 值 |
|---|---|
| 方法文件 | **64 份**（`docs/methods/` 含索引 65 檔） |
| 基準組 | **86**（起於 82） |
| provenance | verified **80** / pending **2**（皆卡文獻） / exempt **4** |
| tier | A **51** / B **31** / I **4** |
| 棘輪 | `MAX_PENDING = 2`、★ `MAX_UNDOCUMENTED = **0**`（起於 44） |
| 沙盒測試 | **13 檔、1,458 過 / 5 跳過** |
| 本機測試 | **17 檔全綠**（2026-07-30 實跑，lint／build 回傳碼皆 0） |
| 紅隊 | **R2–R76**，全部有 Kevin 裁決或已修 |
| 其中 L4（真 bug） | **三個**：R6（A1）、R50（A5a）、R60（A6a） |

---

## ★ A6 開工指令 ✅ **已完成（A6a ＋ A6b，2026-07-30）**

> 本節為 A6 執行時的作業指令，**已全部執行完畢**，保留供後續批次參考其做法。
> A6 依 Kevin 裁決拆為 A6a（8 份）與 A6b（6 份），合計 14 份文件、
> 紅隊 R60–R76 共 17 項（1 個 L4、7 個 L2、1 個 L3、8 個 L1）。
> 結果見 §6.5 的 A6a／A6b 條目、§6.6 的 R60–R76、`docs/methods/README.md` 的兩節摘要，
> 以及 `docs/validation-report-v1.md` 的四個 A6 章節。
## ★ 階段總覽（Kevin 2026-07-25 裁決）

| 階段 | 內容 | 詳細規格 | 狀態 |
|---|---|---|---|
| **A** | **方法文件與紅隊盤點** | **§6** | ✅ **完成（2026-07-30）**：64 份文件、R2–R76 全部處置、§6.7 七條判準全達成、§8 已填完 |
| B | 功能波 Wave F1–F8 | §3 | **← 下一個**（最早 2026 年 9 月底）。★ 但**第一批不是 F1**，見 §8.4 的「復工後建議的第一批工作」 |
...
該節必須在階段 A 收尾時填完才算階段 A 結案（已列入 §6.7 判準）。✅ **2026-07-30 已填完。**
...
（★ 注意：這一整段是**方法文件的模板**，在 code fence 內。
  本檔自己的「§8」指的是下方的「休眠與復工」，不要混淆。）
```
...
| R64 | A6a | `spearman_x1_x2` | L1 | ★ **Spearman 的 $p$ 走 $t$ 近似（同 scipy），而 R `cor.test` 預設在無並列時走精確法**。已量化：216 組確定性排列（$n$=6~60、$\rho$ 掃過全域）中**本工具＝R 的 `exact=FALSE` 分支，max 相對差 3.8e−08**；近似/精確比值 **0.288（$n$=6，$p$ 小 3.5 倍）~ 1.005**，方向系統性偏寬鬆，隨 $n$ 迅速收斂（$n\ge20$ 最大偏低 12%，$n\ge40$ 內 4%）。★★ **但 .05 判定翻面 0 組**——與 A5b 的 R58 證據方向相反（R58 量到 1.1% 偽顯著）。原因是小 $n$ 的精確 $p$ 呈粗糙階梯，比值雖大但兩者落在 .05 同側 | **書面化，不改實作、不加 UI 警告**（證據不支持）。★ **前提必須同時寫出**：(a) 全部無並列；(b) R 的 exact 只在 $n\le9$ 是全枚舉，$n\ge10$ 走 AS 89 ⇒ 量到的是「兩種近似的差」；(c) ★★ **有並列的區域完全沒碰到，而 R58 正是在那裡翻船的**，且 R 的 `cor.test` 有並列時會退回近似 ⇒ **R 不能當那一格的權威** | ✅ **Kevin 2026-07-30 核定：書面化即可，不改實作、不加 UI 警告**。理由：R58 是量到 1.1% 偽顯著才加警告，本批是 0%。★ 並列區同日裁決為**誠實標註未量化、列 backlog**（R 的 `cor.test` 有並列時退回近似，不能當那一格的權威） |
| **R65** | A6a | `descriptive_y` | **L2** | ★ **一個非數值字串讓整欄八個統計量全變 NaN**：`descriptive/compute.js` 缺 `.filter(Number.isFinite)` 而 `normality/compute.js` 有 ⇒ 餵入 `[1,2,'abc',4,5]` 時報表印 **n = 5 但其餘八欄全是「—」**，且不說明原因。★ 紅隊第 3 條（同一個判斷有沒有兩套實作）的變體，其中一支明顯是漏了 | 對齊 `normality` 補上過濾（`compute.js:25`）＋2 條行為測試含「全數值資料結果不得被改動」的回歸鎖 | ✅ 已修（L2 當場修）。既有 fixture 零影響 |
...
### 6.7 完成判準 ✅ **七條全部達成（2026-07-30）**

1. ✅ 每個方法都有 `docs/methods/<id>.md`，八節齊全，第 6 節「尚未驗證的部分」不留白——**64 份**，`docs.coverage.test.js` 逐份檢查八節模板
2. ✅ **2026-07-30 完成**：索引 64/64 連結齊全，且「側欄模組 → 方法對照」一節涵蓋 **28 個已實作模組 ＋ 17 個 COMING_SOON**。★ 順帶抓到兩份**沒有專屬側欄模組**的文件（`levene`、`tukey-hsd`）——它們是別人的前提檢核與事後比較，**只照側欄清單盤點會漏掉**，而 Tukey 那一支正是 R50 的所在
3. ✅ 專案 README 有連結進去（A1 完成）
4. ✅ §6.6 的 **R2–R76** 全部有 Kevin 裁決或已修
5. ✅ **已完成（2026-07-29，A3c）**：`tests/docs.coverage.test.js` 上線。六條斷言——
   八節模板完整性、README 索引涵蓋、**未被任何第 6 節引用的基準組數棘輪
   （`MAX_UNDOCUMENTED` 只能往下調：44 → 36 → 27 → 18 → 16 → 11 → 8 → ★ **0**，2026-07-30 歸零）**、PLS 側（A1–A3）不得掉出涵蓋、
   文件不得引用不存在的基準鍵。★ 上線第一天即抓到 `pls_scheme_centroid`／`pls_scheme_factorial`
   兩組未被任何文件承認（已補）。**寫完一批方法文件就把 `MAX_UNDOCUMENTED` 調降到新的實際值。**
6. ✅ 全套測試綠燈：沙盒 **13 檔 1,458 過 / 5 跳過** ＋ Kevin 本機 **17 檔全綠**（2026-07-30 實跑，三項回傳碼皆 0）
7. ✅ **2026-07-30 填完**：§8.3 的兩個空格、§8.4 的完整快照（含三個 L4 的共同結構、刻意不處理的六項及其理由、復工第一批工作的分流）、§8.5 加寫第四句
...
| 檢查 | 指令 | 休眠當下（2026-07-30）的預期值 |
|---|---|---|
| 沙盒 node 測試（**13 檔**） | `npx vitest run --exclude "**/ui.*.test.jsx"` | ★ **1,458 過 / 5 跳過**（5 個 skip 全在 `compare.test.js`：`mann_whitney_small.pExact` ＋ `cfa_2factor` 的四欄，皆為明文記錄的慣例差異） |
| lint | `npx eslint src tests` | 0 problems |
| build | `npx vite build` | **615 modules transformed**（★ 沙盒因缺 lightningcss 的 Linux 原生二進位而無法跑完 CSS 壓縮，那是環境問題不是程式問題） |
| fixture 重生可重現 | 備份 → `python3 tests/generate_reference.py` → diff | **86 組逐位元相同**；沙盒需 `pip install scipy statsmodels pingouin factor_analyzer semopy plspm scikit-posthocs scikit-learn --break-system-packages` |
| 本機全套（Kevin） | 雙擊 `A6驗收.bat` | ★ **17 檔全綠**（13 檔 node ＋ 5 檔 jsdom，其中 `docs.coverage` 為 A3c 後新增）、lint 0、build 成功。2026-07-30 實跑三項回傳碼皆為 **0** |

★ **沙盒跑不動的 5 檔 jsdom**（`ui.errorBoundary`／`ui.modal`／`ui.smoke`／`ui.toast`／`ui.transformDialog`）
只能在 Kevin 本機驗；動到 `src/analyses/**` 或 `src/i18n/**` 就一定要請他跑。

對不上就**先查清楚為什麼**再往下做——休眠期間唯一會變的是外部套件版本，
數字對不上通常代表某個第三方套件改了預設值，那本身就是要記進 validation-report 的事。

### 8.4 休眠當下的狀態快照（2026-07-30 填）

**階段 A 完成日期**：2026-07-30（A1 起於 2026-07-26，共五天）

| 項目 | 值 |
|---|---|
| 方法文件 | **64 份**（`docs/methods/` 含索引 65 檔） |
| `reference.json` 組數 | **86** |
| provenance 狀態 | verified **80** / pending **2** / exempt **4** |
| provenance tier | A **51** / B **31** / I **4** |
| 棘輪 | `MAX_PENDING = 2`、★ `MAX_UNDOCUMENTED = 0` |
| 沙盒測試 | 13 檔、**1,458 過 / 5 跳過** |
| 本機測試 | 17 檔全綠（＋5 檔 jsdom） |
| §6.6 紅隊 | **R2–R76**，全部有 Kevin 裁決或已修 |

**兩組 pending 的身分**：`pls_bca_reference`、`pls_fimix`——**卡文獻取得，非技術問題**。
開放存取管道已窮盡、Kevin 的機構訂閱未涵蓋（2026-07-25 確認）。詳見 §1 的 Session Q2 表。
★ **不接受以替代驗證充當結案**：兩者現行的三重替代驗證只鎖得住「JS 與 numpy 一致」，
鎖不住「公式讀錯」——而 Session Q2 抓到的兩個 bug（`pls_cta` 的臨界值、`pls_pos` 的目標函數）
證明了這個區別是實質的。

**四組 exempt 的身分**：`pls_copula_inputs`／`pls_fimix_inputs`／`pls_mga_perm_inputs`／`pls_pos_inputs`
——純輸入型 fixture（注入的 permutation／bootstrap 索引），不含統計量。

#### ★ 階段 A 的三個 L4（真 bug）

| # | 批次 | 方法 | 一句話 |
|---|---|---|---|
| **R6** | A1 | `pls_plsc` | PLSc × pairwise／WPLS 走錯相關矩陣 ⇒ rho_A 低估 0.09–0.15（跨過 .70 判準）、路徑高估 18% |
| **R50** | A5a | `tukey_hsd` | `ptukey` 在 df ≥ 100 系統性失準（df=999 誤差 7.6e−1），$p$ 在 .05 兩側翻面 |
| **R60** | A6a | `ks_lilliefors` | Lilliefors $p$ 的兩個定義域錯誤 ⇒ $n\gtrsim325$ 時顯著樣本被印成 $p=1.000$ |

★ **三個的共同結構**：每一個的「唯一基準點」都恰好落在安全區，
而失效區在**參數空間的另一個方向**——方法組合（R6）、df（R50）、$n$（R60）。
⇒ **這不是巧合，是「用一組示範資料建基準」這個做法的系統性盲點。**

#### ★ 階段 A 期間發現但刻意不處理的事項

| 事項 | 不處理的理由 |
|---|---|
| **Mann-Whitney／Wilcoxon 的精確法**（R57／R58） | 缺口已量化：無並列時危險方向為 0；★ **有並列時 78% anti-conservative、1.1% 偽顯著** ⇒ 已加 UI 警告，實作列 backlog P2 |
| **Spearman 的精確法**（R64） | 216 組掃描 **.05 翻面 0 組** ⇒ 證據不支持改動。★ 但**有並列的區域未量化**，且 R 在該格會退回近似 ⇒ 無現成權威 |
| **`quantile` 三份實作**（R71） | 三份都是 type 7、行為一致；動 `pls.js` 要重跑整個 PLS 套件，成本與收益不成比例 ⇒ 階段 B 抽取 |
| **permutation $p$ 的 ±1 修正兩種慣例** | NCA 側 $\#/P$、MGA／MICOM 側 $(\#+1)/(P+1)$，各有依據；改任一側都會動到已對過第三方的基準 |
| **`psych::ICC` 的不一致**（E109） | 本工具側已由 McGraw-Wong 原式逐位元反證；psych 為何如此**沒查出來**，據實標為未解決 |
| **約 60 項 E 編號的功能缺口** | 屬階段 B/D 範圍，見各方法文件第 8 節末表 |

#### ★ 復工後建議的第一批工作

**不是 Wave F1，而是「把階段 A 的 E 編號分流」**——階段 A 累積了約 60 項 E 編號，
其中有一批是**成本極低、風險極高**的，應該優先於任何新功能：

| 優先 | 內容 | 理由 |
|---|---|---|
| 1 | **補齊「零基準但會進論文」的欄位**：`regression_multiple` 的 beta 三欄（E80，已對 R 驗過）、MANOVA 的 Hotelling／Roy 的 $F$／$p$（E139，已對 R 驗過）、Cronbach 的刪題後 α（E124） | ★ **R 已經驗過，只差寫進 `generate_reference.py`**——成本近乎零 |
| 2 | **補格點基準**：VIF 高共線區（E105）、ICC 的 $k\ge3$（E111）、kappa paradox 區（E119） | ★ 三個 L4 的共同結構就是「單點基準 ＋ 失效區在別的方向」 |
| 3 | **`visualization` 模組登記進 provenance**（E95） | ★ **兩道棘輪目前都掃不到它**——不是防線寫錯，是掃描的母體不含它 |
| 4 | 才是 §3 的 Wave F1 | — |
...
2. **tier B 是風險所在**。休眠當下 **86 組基準有 31 組 tier B**（沒有第三方數值對照）。
   ★ **但階段 A 修正了這句話的一半**：A5a 的 R50 與 A6a 的 R60 兩個 L4 都落在 **tier A**——
   「有第三方對照」保護的是**那一個基準點**，不是整個參數空間。
   ⇒ 新增方法時先問「有沒有可執行的第三方」，**接著問「這個基準點覆蓋了參數空間的哪一點」**。
3. **第三方實作不等於可照抄的數字**。2026-07-25 查到 seminr 的 `predict_pls(reps=)` 根本不生效，
   且其彙總口徑有系統性樂觀偏誤；2026-07-30 又查到 `psych::ICC` 把三個型別印成同一個值、
   與 McGraw-Wong 的原式矛盾。⇒ 查核的價值在於**知道它做了什麼**，然後自己判斷。

★ **階段 A 收尾時加的第四句**：**逐值比對永遠比不到不存在的欄位。**
`compare.test.js` 是一道「欄位對欄位」的防線，它的覆蓋率上限就是基準組的欄位清單。
A6b 一批就出現三個這一型（R72／R73／R74）——加權 kappa 的 CI 上界算出 1.0248 而三道防線全綠，
只因為 `cohen_kappa` 那一組根本沒有 CI 欄位。
⇒ **紅隊時要問的不只是「這個數字對不對」，還有「這個模組有哪些數字沒有進基準」。**

---

## 版本紀錄
- v2.24（2026-07-30 同日）：**★★ 階段 A 結案**——§6.7 的**七條判準全部達成**，專案進入休眠。
  ★ **判準 2（28 個側欄模組 → 方法對照表）完成**，並順帶抓到一件事：
  **`levene` 與 `tukey-hsd` 兩份文件沒有專屬的側欄模組**——它們是別人的前提檢核與事後比較，
  **只照側欄清單盤點會漏掉這兩支**，而 Tukey 那一支正是 A5a 的 L4（R50）所在。
  ⇒ **對照表的價值不在「每個模組都有文件」，在於它會逼出「不屬於任何模組的方法」。**
  ★ **判準 7（§8 休眠與復工）填完**：§8.3 補上兩個空格與完整的基準線預期值；
  §8.4 寫進狀態快照、**三個 L4 的共同結構**、**刻意不處理的六項及其理由**、
  以及**復工第一批工作的分流**（★ 明確寫「不是 Wave F1」——先補那批「R 已驗過、只差寫進
  `generate_reference.py`」的零基準欄位，成本近乎零而風險很高）；§8.5 加寫第四句
  「逐值比對永遠比不到不存在的欄位」，並修正第二句——
  ★ **「tier B 是風險所在」只對了一半：三個 L4 裡有兩個落在 tier A**。
  ⇒ 有第三方對照保護的是**那一個基準點**，不是整個參數空間。
  本檔開頭改寫為休眠交接（含階段 A 留下的四條與最終數字表）。
...
- v2.21（2026-07-30 同日）：**A6a 再交付三份（descriptive／levene／correlation），4 / 8**。
  ...
  前提面板印**紅燈「違反變異數同質」**，而真相是**各組變異數完全相等**——**方向剛好相反**；
  且該分支**永遠是錯的**（$SS_w=0$ 蘊含 $SS_b=0$）。修復時還發現：**只把 Infinity 改成 NaN 而不特判，
  會從紅燈變成綠燈「通過」，同樣是錯的** ⇒ 單／雙因子 ANOVA 的前提列加了第三種中性狀態。
  ⇒ **給 A6b 的第一條：每支開工先跑一次常數欄，三十秒。**
  ★ **R65**：`descriptive` 缺 `.filter(Number.isFinite)`（`normality` 有）⇒ 一個非數值字串讓整欄八個統計量全變 NaN。
  ★ **R67**：相關的 `zeroVariance` 是孤兒欄位；修復時發現**舊旗標對整組配對都成立、兩欄時分不出誰是常數欄**，
  改為分開回報 `xConstant`／`yConstant`。★ **R68**：`descriptive.js` 檔頭一句話裡兩個第三方宣稱，只驗掉一個。
  ★ **三份的獨立重寫全部通過**（mpmath dps=40，不呼叫產生基準的函式）：
  Levene $F$ **1.257e−15**、Pearson $r$ **逐位元相同**、Spearman $\rho$ 2.089e−16、偏度／峰度 4.5e−15 / 2.2e−15。
  `MAX_UNDOCUMENTED` **16 → 11**；方法文件 **51 → 54 份**；沙盒 **13 檔、1,425 過 / 5 跳過**。
- v2.20（2026-07-30 同日）：**08 號補跑回收——R60 結案，Spearman 慣例分歧量化（R64）**。
  ★★ **R60 取得真正的獨立證人**：`nortest::lillie.test` 走第三套 dispatch（DW ＋ Stephens），
  11 組探針的 $D$ **逐位元相符**；$p$ 在**決策區（$p<0.1$，含五組 clampzone）max 絕對差 1.5e−8**
  ——連 `likert_n1000` 的 $2.8953\times10^{-114}$ 都對上；$p>0.1$ 區 max 8.9e−3（慣例差異）。
  ⇒ **一致與不一致的分界線正好落在 DW 近似的定義域邊界上，這本身就是 R60 診斷正確的證據。**
  ★ **R64（待裁決）**：Spearman 的 $t$ 近似 vs R 的精確法，216 組確定性排列量化——
  本工具＝R 的 `exact=FALSE` 分支（3.8e−08）；比值 **0.288（$n$=6，$p$ 小 3.5 倍）~ 1.005**，
  方向偏寬鬆但隨 $n$ 收斂（$n\ge20$ 偏低 ≤12%）；★★ **.05 翻面 0 組**，與 R58 的 1.1% 偽顯著相反
  ⇒ **建議書面化即可，不改實作**。⚠️ **前提是全部無並列，而 R58 正是在有並列時翻船的**，
  且 Spearman 有並列時 R 會退回近似 ⇒ **R 不能當那一格的權威，該格仍空白**。
  ★ 另記：`n4_bigD` 被 `lillie.test` 的 $n\ge5$ 下限擋掉 ⇒ **本工具算得出、R 拒算的 $n=4$ 那一格**
  永遠拿不到第三方對照，保證只到「與 statsmodels 一致」。
- v2.19（2026-07-30 同日）：**07 號 R 抽驗回收，五段銷帳、一段陣亡**。
  ★ **三項結案**：`descriptive.js` 檔頭「與 e1071 type=2 一致」的斷言**實跑核實**
  （type 1/2/3 分別 0.4095/**0.4201**/0.3993，峰度 0.5046/**0.6564**/0.3887——**峰度三種算法差 1.7 倍**）；
  Levene 兩個 center 慣例都對上（本工具＝Brown-Forsythe，SPSS 用 mean 版）；
  ★★ **階層迴歸的手算 $\Delta F$／$\Delta p$ 對上 R 的 `anova()`（0.7321 / 0.4855）
  ⇒ §0 品質規範點名的「手算基準與 JS 出自同一次理解」結構性弱點，在本組結案**。
  ★ **兩項新開出**：**E80**——`regression_multiple` 的**標準化係數 beta 三欄零基準**
  （UI 有、fixture 沒有），實測與 R 相符到全部印出位數，建議補進基準；
  **相關係數的 95% CI 完全未實作**（`src/` 內無 Fisher $z$），APA 7 剛需，R 值已記入報告備用。
  ⚠️ **兩項未結案**：(1) **R60 仍無真正的獨立證人**——07 號只對到 $n=60$、$p\approx0.5$
  那一點（0.5161 vs `nortest` 0.4755，屬預期內的 dispatch 差異），而兩邊都離 .05 很遠；
  (2) ★ **Spearman 的慣例分歧量到一點且方向偏寬鬆**（無並列 ⇒ R 走精確法 1.705e−05，
  本工具 $t$ 近似 1.252e−05，**小 1.36 倍**），與 A5b 的 R58 同型。
  ★ **[2b] 段陣亡的成因值得記**：`nortest::lillie.test` 要求 $n\ge5$ 而第一組探針 $n=4$，
  失敗值填邏輯型 `NA` 讓 `formatC` 回報 `unsupported type`，加上整個迴圈包在單一 `tryCatch` 裡
  ⇒ 一組帶走十二組。**這與當天剛開出的 R60 是同一型錯誤——呼叫第三方時沒問它的定義域**，
  只是對象從本工具變成我自己寫的驗證腳本。⇒ 後續抽驗腳本兩條：**先問 $n$ 下限、逐項 `tryCatch`**。
  已修 07 並代產 `08_a6_rerun.R` ＋ `跑A6補跑.bat`（只跑未取得的兩段，含 Spearman 的 216 組掃描）。
