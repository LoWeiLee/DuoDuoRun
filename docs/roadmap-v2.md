...
**★ 現在的下一步：階段 A 的 A1 續作**（Kevin 2026-07-25 裁決；2026-07-26 已開工）。
A1 已交付 1 / 10（`pls_basic`），模板規格已定案、紅隊 R2–R5 已修完。下一步接 A1 其餘 9 個方法群。
...
**A1 — PLS 測量與估計核心（tier B 密集）** 🔄 進行中（1 / 10，2026-07-26）
已完成：`pls_basic`（`docs/methods/pls-basic.md`，含四項紅隊發現 R2–R5，Kevin 當日核定並全部修完）。
★ 模板已由 Kevin 定案（2026-07-26）：照 `pls-basic.md` 的規格展開，其中**「不看程式碼、依文件描述獨立重寫一次」**
（該檔第 8 節 R1）為每份文件的建議步驟——它是唯一能證明「文件寫的公式就是產生基準值的公式」的做法。
