...
功能開發（階段 B）尚未動工。
最近一次沙盒驗收：**1,084 過、6 跳過、8 檔全綠**（2026-07-29，A3b 交付後）。
最近一次本機全套驗收：**1,246 過、6 跳過、13 檔全綠**（2026-07-29，A3a 收尾後）——
★ **A3b 動到 `src/lib/stats/pls.js`、`src/analyses/pls/**` 與 `src/i18n/**`，本機 jsdom 5 檔需補跑**
（新增 3 條 UI 測試，沙盒未執行過）。

**★ 現在的下一步：階段 A / A3 的最後一批（A3c，4 份）**。
**A1、A2 各 10 / 10 已交付；A3 已交付 6 / 10**——A3a（`pls-mga`、`pls-micom`、`pls-itcriteria`）
與 A3b（`pls-predict`、`pls-ipma`、`pls-cipma`），皆 2026-07-29。
紅隊累計 **32 項（R2–R32）全部處置完畢**。
...
1. **寫第 7 節前先跑** `grep -rn "<欄位名>" src/ | grep -v src/lib/stats/`。
   ...
   `provenance.test.js` 管登記、`compare.test.js` 管數值，**沒有一道管可見性**。
2. **比對「說明文字承諾了什麼」與「工具實際做了什麼」**。A3b 的 R32 是新的一類：
   說明寫四級判讀、實作只做三級——**不是有沒有，是級數不符**。
   讀第 2／3 節時把每一句「本工具會…」當成待驗證的斷言。
...
**A3 — PLS 進階分析（W5／W6，tier B 密集）** 🔄 **進行中（6 / 10 交付）**
