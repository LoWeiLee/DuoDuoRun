...
### 五、仍待 SmartPLS 4 抽驗（2 項，不擋開發）

1. **d_G 的對數底數。** 本工具用 ln：飽和 1.046563／估計 1.098801；cSEM 用 log₁₀：0.197394／0.207247。
   比值 5.30190 = (ln 10)²，七位有效數字吻合，確定為底數差異。判準為 bootstrap 分位數（相對比較），
   **統計結論不受影響**，但報表數字會與 SmartPLS／cSEM 差 5.3 倍。
   抽驗方式：SmartPLS 4 的 Model Fit 報表讀 d_G（Saturated／Estimated）；同表的 SRMR
   應為 0.0976／0.1044（已知對齊，可當操作正確性的 sanity check）。
2. **Henseler MGA p 的公式。** 本工具用偏誤校正 draws（2θ̂ − θ*）：P(θ_M ≤ θ_F) = 0.1129；
   cSEM 用原始 draws：0.0389（其報表 0.9678 為 group1 = F 的鏡射）。三個不同種子下本工具皆為
   0.113–0.120，非蒙地卡羅噪音，屬系統性公式差異。抽驗方式：SmartPLS 4 的 PLS-MGA p-value。
