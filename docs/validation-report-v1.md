...
6. `tests/a6.behavior.test.js` 28 條行為鎖
...
沙盒 **13 檔全綠、1,425 過 / 5 跳過**（`a6.behavior.test.js` 39 條）；
...
## 階段 A / A6a 收官（2026-07-30 同日）：visualization ＋ 迴歸三支，8 / 8

### 一、獨立重寫（mpmath dps = 40，自寫 Gauss-Jordan 求逆）

**不呼叫 `statsmodels.OLS`，也不呼叫 `variance_inflation_factor`**；
$F$ 與 $t$ 的尾機率一律走正規化不完全 beta。

| 組 | 欄數 | max 相對差 |
|---|---|---|
| `regression_simple` | 9 | **6.608e−15** |
| `regression_multiple`（含 VIF 三欄） | 14 | **1.350e−14**（VIF 為 4.5e−16 ~ 2.5e−17） |
| `regression_hierarchical` | 5 | **2.780e−14** |

★ **另補上一條此前不存在的交叉鎖**：`simpleLinearRegression`（閉式解）與
`multipleRegression`（解 $(\mathbf X'\mathbf X)^{-1}$）是**兩套獨立的 OLS 實作**，
$k=1$ 時應同值——而**沒有任何測試檢查這件事**（紅隊第 3 條在演算法層的版本）。
實測：本資料集 max 相對差 **2.9e−13**、200 組確定性資料 **5.2e−15**，**無偏離**，已入 `a6.behavior.test.js`。

### 二、R69：同型第五次，而症狀最刺眼的一次

| 情形 | 舊版報表 |
|---|---|
| ★ 多元迴歸完美配適 | $R^2=1.0000$、$R^2_{\text{adj}}=1.0000$、$p<.001$，而 **$x_1$ 的 $t$ 印成 `2312738254615615.50`** |
| ★ 多元迴歸 $y$ 常數 | $R^2$／$F$ 印「—」，但 **$x_1$ 的 $p=.001$ 顯著**——$SS_{\text{res}}=1.3\times10^{-28}$、$SS_{\text{total}}=0$，係數純屬浮點雜訊 |
| ★ 簡單迴歸 $y$ 常數 | **截距 $p=0$ 印「< .001」綠燈**，其餘印「—」 |
| ★ 簡單迴歸完美配適 | $R^2=1.000$、斜率 $p<.001$，而 $t=\infty$ 印「—」——看起來只是排版空白 |

★★ **前四次都是「印出無意義的『—』或綠燈」，這一次是印出一個 2.3 千兆的數字。**
`fmtNum` 對 $10^{15}$ 級的有限值不會退回「—」，它就老老實實地格式化到小數點後兩位。
⇒ **格式化函式的防線只擋 `Infinity` 與 `NaN`，擋不住「有限但荒謬」的值。**

★ **簡單迴歸這一側有結構性的理由**：多元迴歸靠 $(\mathbf X'\mathbf X)$ 不可逆擋掉常數欄與完全共線，
而**簡單迴歸根本沒有矩陣可解**——同一道防線對它完全無效。
⇒ **「這個模組有沒有那道防線」不能從「同族的另一個模組有」推論出來。**

**可達性**：問卷研究裡把**總分對它自己的分量表**迴歸就會得到接近完美的配適，
而報表看起來就是「找到了一個極強的關係」。

**處置**：引擎新增 `perfectFit`（判準用相對值 $SS_{\text{res}}/SS_{\text{total}}<10^{-20}$，
因為浮點下完美配適的 $SS_{\text{res}}$ 是 $10^{-27}$ 級而非恰好 0）＋簡單迴歸補 `zeroVarianceY`
（多元本來就有，**但是孤兒欄位**）；三支各加警告框並**取消 $p$ 的顯著性燈號**；
i18n 中英 × 三命名空間 × 三鍵 ＝ 18 個字串；7 條測試含回歸鎖。
★ 順帶修掉 `maxVif`：引擎算好回傳了，`Result.jsx` 卻**又用 `Math.max(...)` 算一次**（同一判斷兩套實作）。

### 三、★ R70：這一項是我自己的錯誤，值得完整記下來

`descriptive.md` 寫了「四分位數完全未實作」、`correlation.md` 寫了「沒有散布圖」。
**兩句都是錯的**：

| 我寫的 | 實情 |
|---|---|
| 四分位數未實作 | `viz/boxStats.js:12–22` 就有 `quantile()`，箱型圖用它算 Q1／中位數／Q3／IQR |
| 沒有散布圖 | `visualization` 模組的 `scatter` 一直都在 |

★ **成因**：我 `grep` 的範圍是 `src/lib/stats/` 與該方法自己的 `src/analyses/<module>/`，
**不含 `src/lib/viz/` 與其他模組**——然後就對「整個工具有沒有這個功能」下了結論。

★ **這與 R63 是一體兩面**：R63 是**檔頭宣稱「與 nortest 一致到小數第 3 位」而零證據**，
R70 是**文件宣稱「沒有這個東西」而它其實有**。
⇒ **「宣稱不存在」和「宣稱有證據」需要同等程度的查證。**
第 6 節的「尚未驗證的部分」如果寫錯方向，它的誠實度就變成負的——
**讀者會以為某個功能缺席而去別處找，或以為某個前提無從檢查而放棄檢查。**

**處置**：兩份文件更正並**保留更正痕跡**（不是默默改掉）；缺口重新定義為
「敘述統計表不顯示 IQR」與「相關頁面不引導使用者去看散布圖」——**這兩句才是真的**。

★ **順帶把一個零基準量結案**：`boxStats` 的分位數慣例確認為 **R 的 `quantile(type = 7)`**
（$h=(n-1)p$ 加線性內插），對照 07 號抽驗的 R 值：
$Q_1$ 絕對差 **7.1e−15**、$Q_3$ **逐位元相同**。

### 四、R71：`quantile` 在專案內有三份

| 位置 | 狀態 |
|---|---|
| `viz/boxStats.js:12–22` | 9 行 |
| `viz/binning.js:13–22` | ★ **與上者逐字元完全相同** |
| `stats/pls.js:140–148` | 寫法不同（多一個 clamp），公式相同，註解明寫「R type 7」 |

★ **兩份逐字元相同的複製在同一個目錄裡**——不是跨層複製。
三份都是 type 7 ⇒ 目前無行為分歧，但這正是 A5b 習慣 9 警告的形狀
（`cfa.js` 當年也「只是」多養一套常態 CDF，而那一套在 R55 被發現會讓尾機率塌成 0）。

**處置**：書面記錄，本批不重構——動 `pls.js` 要重跑整個 PLS 回歸套件，成本與收益不成比例。
建議階段 B 抽取為 `src/lib/stats/quantile.js`（E96）。

### 五、★ `visualization` 是階段 A 唯一一份「零基準模組」的文件

它**沒有基準組、沒有 adapter、沒有任何測試**，`provenance.json` 也沒有登記。
而它畫出來的數字（四分位數、鬚位、離群值、分箱寬度）**會被使用者讀進論文**。

★★ **最值得記的一項發現（E95）**：**兩道棘輪都掃不到它**。
`provenance.test.js` 掃的是 `reference.json` 的鍵、`docs.coverage.test.js` 檢查的是
「基準組有沒有被文件第 6 節提到」——**兩者對「一個完全沒有基準的模組」都無效**。

⇒ 這是 A4 的 R41（防線的漏收）在**模組層**的版本：
**不是防線寫錯，是防線掃描的母體不含它。**
R41 當時漏的是 16 個含 `>`／`=` 的錯誤碼，這次漏的是**一整個側欄模組**。

★ 順帶量到一個真正的慣例差異：**直方圖預設 Freedman-Diaconis，而 R `hist` 預設 Sturges**
⇒ 同一批資料在兩邊會畫出不同的直方圖，而 UI 上沒有任何說明（E93）。

### 六、A6a 全批總結

| 項目 | 數字 |
|---|---|
| 方法文件 | **8 份**（`normality`／`descriptive`／`levene`／`correlation`／`visualization`／迴歸三支） |
| 紅隊 | **R60–R71 共 12 項**：1 個 **L4**（R60）、5 個 L2（R61／R65／R66／R67／R69）、6 個 L1 |
| 基準組 | 85 → **86**（新增 `ks_lilliefors_grid` 84 欄） |
| `MAX_UNDOCUMENTED` | 18 → **8**（剩下全部落在 A6b） |
| 沙盒測試 | 12 → **13 檔**，**1,435 過 / 5 跳過**（`a6.behavior.test.js` 49 條） |
| 行號重驗 | **198 處**（三批分別修正 4／8／16 處） |
| R 側抽驗 | 07 號六段（五段成功）＋ 08 號兩段 |

★ **本批最該帶進 A6b 的三條**：

1. ★★ **「零變異／退化值」已經連中五次**（A4 R40-i／A5a R51／A6a R61／R66／R69），
   分屬五個模組。**每支開工先跑一次常數欄與完美配適，三十秒。**
   ★ 而且要注意 R69 的教訓：**格式化函式只擋 `Infinity` 與 `NaN`，擋不住「有限但荒謬」的值**。
2. ★ **同一族的模組不共用防線**：多元迴歸靠 `singular-matrix` 擋掉的東西，
   簡單迴歸完全沒有。**「另一支有」不是「這一支也有」的證據。**
3. ★ **寫「未實作」之前先確認 `grep` 的範圍**（R70）。
   第 6 節的誠實度是雙向的——寫錯「沒有」跟寫錯「有」一樣糟。
## 階段 A / A6b（2026-07-30 同日）：六份交付，★★ 階段 A 的方法文件目標達成

### 一、★★ 本批三個紅隊項的共同點，就是本批最該記住的一句話

**逐值比對永遠比不到不存在的欄位。**

| # | 缺的欄位 | 後果 |
|---|---|---|
| **R74** | `cohen_kappa` 沒有 **SE 與 CI** | 加權 CI 用錯變異數公式，上界算出 **1.0248** |
| **R72** | `cluster_*` 沒有**退化旗標** | 30 個相同的觀察值被切成三群 |
| **R73** | `manova` 沒有**奇異矩陣旗標** | Wilks 缺席而 Pillai 印出 $p=1.000$ 的「正常不顯著」 |

⇒ 這與 A5b 的 R56（Dunn 事後比較零基準）是同一型，而本批**一次出現三個**。
★ **`compare.test.js` 是一道「欄位對欄位」的防線——它的覆蓋率上限，就是基準組的欄位清單。**

### 二、R74（L3）：加權 kappa 的信賴區間

修復前 `kappa.js:172`：`varCI = po(1−po)/(n(1−pe)²)` ——**未加權** kappa 的漸近變異數。

| 加權 | 舊 SE | 正確 SE | 舊 CI 上界 |
|---|---|---|---|
| none | 0.08524 | 0.08567 | 0.6465 |
| linear | 0.09403 | 0.06445 | 0.8399 |
| **quadratic** | **0.11372** | **0.04715** | ★ **1.0248** |

★★ **兩層問題，第二層比第一層重要**：

1. **值域錯**：$\kappa\le1$ 而報表印出 1.025（同 A5b 的 R54，$\varepsilon^2$ 印出負數）
2. ★ **方向也錯**：加權放寬了「一致」的定義 ⇒ $\kappa$ 較高、抽樣變異**較小**。
   舊公式對三種加權用同一條式子，讓 quadratic 給出**最大**的 SE——**連單調方向都反了**。
   ⇒ 我把這條方向性寫成了測試（`seQuad < seLinear < seNone`），**它比逐點比對更難繞過**。

**處置**（Kevin 核定選項 1）：改用 Fleiss, Cohen & Everitt (1969) 的加權大樣本變異數
＋ CI 夾在 $[-1,1]$ ＋ **基準組 3 → 12 欄**（權威為 `statsmodels.stats.inter_rater.cohens_kappa`）。

★ **修正後四方一致**：本工具 ＝ statsmodels（≤ 3.2e−10）＝ R `psych::cohen.kappa`（1e−9）
＝ FCE 原式沙盒手算。`reference.json` 重生後**其餘 85 組逐位元不變**。

### 三、R72（L2）：集群「無中生有」——同型第五次，但性質不同

| 情形 | 舊版報表 |
|---|---|
| 30 個**完全相同**的觀察值 + Ward + $k=3$ | 切成 **8/8/14 三群**，三個中心點一模一樣 |
| 全常數 + k-means | **[30, 0, 0]**——兩個空集群 |
| 只有 2 個相異點 + $k=3$ | **[15, 15, 0]** ＋ **silhouette = 1.0000（完美）** |

★★ **前四次（A4 R40-i／A5a R51／A6a R61／R66）都是「把數學上無定義的結果印成有意義的判定」，
這一次是「憑空造出三個集群」。** 而最誤導的是最後一個：**空集群配完美輪廓係數**——
使用者看到 silhouette = 1.000 會認為分群品質極佳，真相是資料裡只有兩種值。

**處置**：判準取「**相異觀察值數 < $k$ 或有空集群**」——一條涵蓋兩種病徵。
★ 修復時發現輪廓係數的判讀在**三個地方**各算一次（`QualitySection` 卡片、解讀段、主元件），
三處都要改——**這本身就是「同一判斷多套實作」的又一個實例**。

### 四、R73（L2）：MANOVA 的 Wilks 誠實、其他三個不誠實

★ **成因是實作路徑不同**：Wilks 走 $|\mathbf E|/|\mathbf E+\mathbf H|$，行列式為 0 時誠實回 NaN；
Pillai／Hotelling-Lawley／Roy 走 `eigenvaluesEinvH` 的**數值特徵值路徑**，$\mathbf E$ 奇異時仍算得出東西。

依變項全為常數時報表印出 **$V=0.000$、$F=0.000$、$p=1.000$、$\eta^2=0.000$**
——一個看起來完全正常的「不顯著」結論。

★ **使用者的自然反應**：看到「Wilks: —」以為只是某個統計量算不出來，**然後去讀旁邊的 Pillai**。
⇒ 警告文字因此**直接點破那個陷阱**，明說「Pillai、Hotelling-Lawley 與 Roy 仍會印出數字，
那些數字同樣無意義」。

### 五、兩件「命名／慣例」結案

**1. ICC 六個欄位的命名——由公式層結案**

「`icc21` 是不是真的對應 McGraw-Wong 的 ICC(2,1)」此前只是一個名字，而**選錯型別是 ICC 最常見的誤用**。

依 McGraw & Wong (1996) 的六條原式獨立重算（$MS_R=2.00339$、$MS_C=0.03333$、
$MS_E=0.21977$、$MS_W=0.21667$）：**六欄逐位元相同（0.00e+00）** ⇒ **命名正確**。

⚠️ **但 R `psych::ICC` 對不上**：它把 ICC1 = ICC2 = ICC3 印成同一個值（0.80481040），
而 §3.2 的公式在 $MS_W\ne MS_E$ 時**必然給出不同的值**——psych 的輸出與 McGraw-Wong 矛盾，
最大差 3.1e−3。

★ **本文件的立場**：本工具側有**更強的證據**（教科書原式逐位元反證，比「與某個套件一致」更強）。
psych 為何如此**沒有查出來**，據實記為 E109，下一步是 `psych::ICC(x)$summary` 比對 ANOVA 表。
⇒ **這是「不確定就標不確定」的實例——不因為自己這側有強證據就把對方的異常掃到地毯下。**

**2. ★★ sklearn 的 `linkage='ward'` ＝ R 的 `ward.D2`**

| 方法 | 各群大小 | WSS |
|---|---|---|
| **`ward.D2`** | **17, 18, 25** | **94.76284469** |
| `ward.D` | 12, 23, 25 | 93.42952662 |
| **本工具** | **17, 18, 25** | **94.76284469**（且 **60 筆標籤逐一相同**） |

**這個問題本專案此前從未回答過**，而答錯的話整組分群標籤都不一樣。

### 六、獨立重寫（六支全數通過）

**不呼叫產生基準的那些函式**——`statsmodels.Logit`／`pingouin.cronbach_alpha`／
`pingouin.intraclass_corr`／`sklearn.cohen_kappa_score`／`statsmodels.MANOVA`／`sklearn.KMeans` 一個都沒碰。

| 組 | max 相對差 |
|---|---|
| ★ `icc`（六欄） | **0.00e+00（逐位元相同）** |
| ★ `cluster_ward_k3`（自寫 Lance-Williams 遞推） | **0.00e+00**，標籤逐一相同，$TSS=177$ 相符 |
| `cohen_kappa`（12 欄，含 FCE 變異數） | 0.00e+00 ~ 3.5e−15 |
| `cronbach_alpha`（兩組） | 2.9e−16 |
| `manova`（八欄，自建 $\mathbf H$／$\mathbf E$、自解特徵值、自推 Rao $F$） | 1.9e−14 |
| `logistic_regression`（12 欄，自寫 IRLS） | 4.2e−9 |

★ **`logistic_regression` 的 4.2e−9 已查明成因**：`llNull` 本工具與重寫都用閉式解
$n[\bar p\ln\bar p+(1-\bar p)\ln(1-\bar p)]$，而 statsmodels 是**迭代求 null 模型**
——差異來自它的收斂容差，不是公式。連帶讓 `lrStat`／`mcFadden`／`nagelkerke` 停在 4e−9。
★ 而 `auc` 是**逐位元相同**——重寫用 Mann-Whitney $U$ 形式自算，完全沒碰 `sklearn.roc_auc_score`。

### 七、★ 順帶記一個我自己的錯（09 號腳本）

09 號 R 抽驗的 ICC 與 MANOVA 兩段**比對無效**，因為我用錯了變項：
ICC 的基準是 **rater1 + rater2**（兩位評分者）而我寫成 cond1~cond3；
MANOVA 的基準是 **y + x1 + x2 ~ group3** 而我寫成 `cbind(x1, x2)`。

★ **成因**：我照 roadmap 的文字描述寫腳本，**沒有回去看 `generate_reference.py`**。
這是本專案第二次犯同型的錯（07 號的 [2b] 段是沒問過 `lillie.test` 的 $n$ 下限）。

⇒ **給後續抽驗腳本的一條**：**變項與參數要從 `generate_reference.py` 抄，不能從文字描述重建。**
已代產 10 號補跑，用對變項後兩段全部對上。
...
| `MAX_UNDOCUMENTED` | ★★ **0**（86 組基準 100% 被第 6 節點名；該棘輪從 A3c 上線時的 44 一路降到 0） |
| 基準組 | **86**（`cohen_kappa` 由 3 欄擴充為 12 欄） |
| A6 兩批的紅隊 | **R60–R76 共 17 項**：1 個 **L4**、7 個 L2、1 個 L3、8 個 L1 |
| 沙盒測試 | **13 檔、1,458 過 / 5 跳過**（`a6.behavior.test.js` 63 條） |

★ **剩下的兩條判準**：§6.7 判準 2（28 個側欄模組 → 方法對照表）與判準 7（§8.4 休眠快照）。
