...
| 項目 | 本工具 | 他方 | 影響 |
|---|---|---|---|
| ★ Lilliefors $p$ 的路線 | statsmodels 的 **approx**（近似優先、$p>0.1$ 查表） | statsmodels 另有 **table**（一律查表）；R `nortest::lillie.test` 用 **DW ＋ Stephens** 的第三套 dispatch | ✅ **2026-07-30 已量到一點**：$n=60$、$p\approx0.5$ 時本工具 0.5161 vs R 0.4755（**差 0.0406**）——落在近似無效區，屬預期內的 dispatch 差異。⚠️ **決策區（$p\in[.01,.10]$）仍未量化**，見尚未驗證第 2 項 |
...
2. ✅ **R 端已對照，R60 結案。** 2026-07-30 的 `07` §2a ＋ `08` §A：
   $D$ 在 11 組探針逐位元相符；$p$ 在**決策區（$p<0.1$）max 絕對差 1.5e−8**、
   在 $p>0.1$ 的區域 max 8.9e−3（慣例差異，見上表第 12 道）。
   ★ **殘餘的未驗證處**：$p>0.1$ 時「跟隨 statsmodels 的表」與「跟隨 Stephens」哪一個比較接近真值，
   **沒有第三方裁決**——兩者都是對同一個未知分布的近似，本工具只是選了其中一邊。
3. ✅ **決策區的三方差異已量化**：本工具與 `nortest` 在 $p<0.1$ 差 1.5e−8（見上）。
   ★ **但第三方只有兩家**——statsmodels（移植來源）與 R `nortest`。**SPSS 從未對照過**，
   而 SPSS 的 Lilliefors 修正是另一套實作，慣例差異未知。
...
3. ✅ **與 R `nortest` 的差異已量化**（決策區 1.5e−8、$p>0.1$ 區 max 8.9e−3）。★ 但 **SPSS 仍未對照**
