...
→ `src/lib/stats/pls.js:2286–2291`
...
→ `pls.js:2293–2305`（逐欄算平均 `2295–2303`、寫入 `2304–2305`）

然後**整個模型重新估計**，包含**重新標準化**（`pls.js:2306`）。
...
→ `pls.js:2318–2330`（$\hat y$ `2321–2322`、$z^{\text{true}}$ `2324`、$\hat z$ `2325`、
...
3. **只有內生構念有 Q²**（外生構念沒有結構預測可用）→ `pls.js:2277`。
...
$\text{SSO}\le10^{-12}$ 時 $Q^2$ 回 `null`（`pls.js:2335`）。

## 4. 假設前提與本工具的檢核方式

| 前提 | 本工具怎麼檢核 | 違反時的行為 | 位置 |
|---|---|---|---|
| **不與 pairwise deletion 併用** | 檢查 `options.missing` | **硬擋** `blindfold-pairwise-conflict`，訊息說明「無法區分被挖掉的格子與原本就缺的格子」 | `pls.js:2236–2241` |
| 不含調節／高階構念 | 檢查 model | **硬擋** `q2-not-supported` | `pls.js:2242–2248` |
| $D\ge2$ 且為整數 | 型別檢查 | **硬擋** `bad-omission-distance` | `pls.js:2250–2252` |
| **$n$ 不是 $D$ 的整數倍** | $n\bmod D=0$ 時警告 | 警告（不擋），引 SmartPLS 慣例建議改用其他 $D$ | `pls.js:2262–2264` |
| 完整資料可收斂 | 先跑基準估計 | `blindfold-failed`，訊息說明「完整資料的 PLS 估計即未收斂」 | `pls.js:2271–2274` |
| 每一輪補值後仍可估計 | 逐輪檢查零變異與收斂 | `blindfold-failed`，**指名構念與輪次** | `pls.js:2307–2313` |
| PLSc 不適用 | 強制 `consistent: false` | 一律以 composite 估計計算 | `pls.js:2253` |
...
| UI 欄位 | 對應公式 | 程式碼 |
|---|---|---|
| 構念的 Q² | 3.3 | `pls.js:2335` |
| SSE | 3.3 | `pls.js:2327` |
| SSO | 3.3 | `pls.js:2328` |
| omission distance | 3.1 | `pls.js:2338`（回傳）、`2250` |
| $n\bmod D=0$ 警告 | §4 第四列 | `pls.js:2262–2264` |
