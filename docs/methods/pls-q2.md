...
→ `src/lib/stats/pls.js:2366–2371`
...
→ `pls.js:2373–2385`（逐欄算平均 `2295–2303`、寫入 `2304–2305`）

然後**整個模型重新估計**，包含**重新標準化**（`pls.js:2405`）。
...
→ `pls.js:2407–2410`（$\hat y$ `2321–2322`、$z^{\text{true}}$ `2324`、$\hat z$ `2325`、
...
3. **只有內生構念有 Q²**（外生構念沒有結構預測可用）→ `pls.js:2357`。
...
$\text{SSO}\le10^{-12}$ 時 $Q^2$ 回 `null`（`pls.js:2415`）。

## 4. 假設前提與本工具的檢核方式

| 前提 | 本工具怎麼檢核 | 違反時的行為 | 位置 |
|---|---|---|---|
| **不與 pairwise deletion 併用** | 檢查 `options.missing` | **硬擋** `blindfold-pairwise-conflict`，訊息說明「無法區分被挖掉的格子與原本就缺的格子」 | `pls.js:2316–2321` |
| 不含調節／高階構念 | 檢查 model | **硬擋** `q2-not-supported` | `pls.js:2322–2328` |
| $D\ge2$ 且為整數 | 型別檢查 | **硬擋** `bad-omission-distance` | `pls.js:2330–2332` |
| **$n$ 不是 $D$ 的整數倍** | $n\bmod D=0$ 時警告 | 警告（不擋），引 SmartPLS 慣例建議改用其他 $D$ | `pls.js:2342–2344` |
| 完整資料可收斂 | 先跑基準估計 | `blindfold-failed`，訊息說明「完整資料的 PLS 估計即未收斂」 | `pls.js:2351–2354` |
| 每一輪補值後仍可估計 | 逐輪檢查零變異與收斂 | `blindfold-failed`，**指名構念與輪次** | `pls.js:2415–2393` |
| PLSc 不適用 | 強制 `consistent: false` | 一律以 composite 估計計算 | `pls.js:2333` |
...
| UI 欄位 | 對應公式 | 程式碼 |
|---|---|---|
| 構念的 Q² | 3.3 | `pls.js:2415` |
| SSE | 3.3 | `pls.js:2407` |
| SSO | 3.3 | `pls.js:2408` |
| omission distance | 3.1 | `pls.js:2418`（回傳）、`2250` |
| $n\bmod D=0$ 警告 | §4 第四列 | `pls.js:2342–2344` |
