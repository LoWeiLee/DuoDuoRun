...
| 慣例 | 用者 | 依據 |
|---|---|---|
| $(N-1)F$ | ★ **本工具**、AMOS、lavaan 的 `likelihood = "wishart"` | Wishart 分布的自由度 |
| $N\cdot F$ | **semopy**（本工具的基準來源）、★ **lavaan 預設**（`likelihood = "normal"`） | 概似比檢定的直接形式 |

★ **2026-07-30 更正**：本表原本把「lavaan 預設」放在 $(N-1)F$ 那一列，**那是錯的**。
Kevin 本機實跑（R 4.6.0 / lavaan 0.7.2）確認 lavaan **預設給 $\chi^2=7.224814$，與 semopy 同側**；
需明示 `likelihood = "wishart"` 才會得到 7.104400（與本工具相對差 5.6e−10）。
⇒ **本工具是相對 semopy 與 lavaan 預設都不同的那一個**，這件事在對外說明時要講清楚。
...
$$\mathrm{Cov}(\hat{\boldsymbol\theta})\approx\frac{2}{N-1}\,\mathbf H^{-1},\qquad
\mathbf H=\nabla^2 F_{\mathrm{ML}}\ \text{（中央差分，}h=10^{-4}\text{）}$$

★ **SE 的慣例有兩個獨立的軸，本工具的組合是「觀察訊息 ＋ $N-1$」**
（2026-07-30 經 lavaan 四種設定的對照確認，見 §6）：

| 軸 | 本工具 | lavaan 預設 | 影響 |
|---|---|---|---|
| 訊息矩陣 | **觀察**（數值 Hessian） | 期望（`information = "expected"`） | 隨指標變動，1.009 ~ 1.046 |
| 樣本數 | **$N-1$**（同 §3.3 的 $\chi^2$ 慣例） | $N$ | 固定 $60/59=1.016949$ |

⇒ 本工具的 se 與 **lavaan 的 `likelihood="wishart"` ＋ `information="observed"`** 相符至 **4.5e−06**
（＝中央差分的截斷誤差量級）。與 lavaan **預設**相比，本工具的 $z$ 小約 3.5%——**偏保守**，
但那是慣例選擇的後果，兩端都是 lavaan 支援的合法設定。
...
2. ✅ **已於 2026-07-30 在 Kevin 本機與 lavaan 對照**（R 4.6.0，`scripts/validation/05_a5b_r_audit.R` §1）。
   結果分三層：
   - ✅ **六個標準化負荷與因子相關完全吻合**（0.698／0.788／0.665／0.669／0.648／0.690、$\phi_{12}=0.452$）
   - ★ **本文件原本寫「lavaan 與本工具的 $\chi^2$ 慣例相同」是錯的**：lavaan 預設
     （`likelihood = "normal"`）給 $\chi^2=7.224814$，與**semopy 相同**；本工具的 7.104400
     比值恰為 $60/59=1.016949$ ⇒ **本工具是相對 semopy 與 lavaan 預設都不同的那一個**
     （$(N-1)\cdot F$ 的 AMOS／Wishart 慣例；lavaan 需 `likelihood = "wishart"` 才會一致）。已更正 §6 的慣例差異表
   - ★ **未標準化負荷差一個固定比例 $\sqrt{60/59}=1.008439$**——與 $\chi^2$ 同源，屬慣例、可解釋
3. ✅ **SRMR 已取得第三方佐證**：lavaan 報 **0.061**，本工具 0.060858（lavaan 僅印 3 位小數，
   到該精度為止相符）。⇒ 從「零對照」升級為「粗對照通過」；★ **尚未取得可逐值比對的位數**
4. ✅ **標準誤（SE／$z$／$p$）已結案：純慣例差異，不是實作偏差**（2026-07-30，`06_cfa_se_probe.R`）。
   把 lavaan 的 `information`（observed／expected）× `likelihood`（normal／wishart）四種組合
   印到 10 位有效數字後，兩個軸**乾淨地分離**：

   | lavaan 設定 | 本工具 ÷ lavaan 的 se 比值 | 判讀 |
   |---|---|---|
   | A：normal ＋ expected（lavaan 預設） | 1.026427 ~ 1.063793 | 兩軸都不同 |
   | B：normal ＋ **observed** | **1.016945 ~ 1.016951（六個指標幾乎常數）** | ★ 剩下的差只有 $60/59=1.0169492$ ⇒ 這一軸純粹是 $N$ vs $N-1$ |
   | C：**wishart** ＋ expected | 1.009320 ~ 1.046063（隨指標變動） | ★ 剩下的差隨指標變動 ⇒ 這一軸是觀察 vs 期望訊息 |
   | ★ D：**wishart ＋ observed** | ★ **0.999995 ~ 1.000002** | ★ **就是本工具的口徑** |

   ⇒ 本工具的 SE ＝ **lavaan 的 `likelihood="wishart"` ＋ `information="observed"`**，
   六個指標的最大相對偏差 **4.5e−06**——而那正是數值 Hessian（中央差分 $h=10^{-4}$）的截斷誤差量級，
   **不是公式錯誤**。$z$ 值亦逐一對上（5.102 vs 5.101996、4.8576 vs 4.857563 …）。
   $\chi^2$ 對 lavaan 的 wishart 版相對差 **5.6e−10**。

   ★ **結論**：本工具較不容易判顯著（$z$ 比 lavaan **預設**小約 3.5%），但那是**慣例選擇**
   （觀察訊息 ＋ $N-1$）的後果，兩端都是 lavaan 支援的合法設定 ⇒ 雙處標註，不改實作。
   已在 §3 與 §6 的慣例差異表寫明；使用者若要與 lavaan 預設對齊，需知道差異來自這兩個軸
5. ★ **SRMR 相符到 4.8e−06，但非逐位元**：lavaan 四種設定都給 0.06085815（彼此一致
   ⇒ SRMR 與上述兩軸無關），本工具 0.06085844336 ⇒ 相對差 **4.79e−06**。
   量級遠小於任何判讀門檻（.08／.10），但**大於 $\chi^2$ 的 5.6e−10**，
   代表不是最佳化收斂殘差，而是 §3.4 的 SRMR 分母慣例可能與 lavaan 略有出入 ⇒ 書面記錄（E71）
6. ★ **因子相關的標準誤本工具完全不報**：lavaan（同 D 設定）給 $\phi_{12}$ 的
   se = 0.1617084648、$z=2.794479$。★ **本工具的 Hessian 裡就有這一格**（`Phi` 用 atanh 重參數化），
   只是沒有 delta method 還原、也沒有出口 ⇒ 補上的成本很低（E72）
5. ★ **因子相關的標準誤本工具完全不報**：lavaan 給 $\phi_{12}$ 的 se = 0.150、$z=3.004$、$p=0.003$，
   本工具只報點估計 0.451915
