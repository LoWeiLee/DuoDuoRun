...
4. ★★ **標準誤（SE／$z$／$p$）對不上 lavaan，且不是尺度可解釋的**（2026-07-30 R 抽驗開出）：

   | 指標 | 本工具 se | lavaan se | 本工具 $z$ | lavaan $z$ |
   |---|---|---|---|---|
   | i1 | 0.147094 | ≈0.1406 | 5.1020 | **5.290** |
   | i2 | 0.153820 | ≈0.1450 | 5.6690 | **5.980** |
   | i3 | 0.149774 | ≈0.1430 | 4.8576 | **5.032** |

   ★ 標準化負荷完全吻合、未標準化只差 1.0084 的固定比例，**唯獨 SE 差約 4%、$z$ 小約 3.5%**
   ——這無法用 $N$ vs $N-1$ 的尺度解釋。方向是**保守**（本工具較不容易判顯著），
   但 se／$z$／$p$ 決定報表上哪些 loading 標星號，屬會直接進論文的量。

   候選成因有三，本工具的做法是「數值 Hessian（中央差分 $h=10^{-4}$）＋ $\mathrm{Cov}=\frac{2}{N-1}H^{-1}$」
   ＝**觀察訊息 ＋ $N-1$**，而 lavaan 預設為**期望訊息 ＋ $N$**：
   (a) 觀察訊息 vs 期望訊息、(b) $N$ vs $N-1$、(c) 數值 Hessian 的截斷誤差。
   ⬜ **`scripts/validation/06_cfa_se_probe.R` 已代產**（雙擊 `跑CFA標準誤診斷.bat`），
   把 lavaan 的 2×2 設定組合印到 10 位有效數字，用以逐一排除。**待 Kevin 執行後才能結案**
