...
| **R59** | ★★ **CFA 的 loading 標準誤對不上 lavaan**：標準化負荷完全吻合、未標準化只差 $\sqrt{60/59}$ 的固定比例，**唯獨 se 差約 4%、$z$ 小約 3.5%**，不是尺度可解釋的。方向保守，但 se／$z$／$p$ 決定哪些 loading 標星號 | ⬜ **未結案**。`06_cfa_se_probe.R` 已代產（雙擊 `跑CFA標準誤診斷.bat`），把 lavaan 的觀察／期望訊息 × $N$／$N-1$ 四種組合印到 10 位有效數字，用以區分「慣例差異」與「數值 Hessian 截斷誤差」 |
