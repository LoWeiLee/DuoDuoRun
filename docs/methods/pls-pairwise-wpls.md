...
| 不與 blindfolding 併用 | `blindfoldPLS` 入口檢查 | **硬擋** `blindfold-pairwise-conflict` | `pls.js:2236–2241` |
| 權重為有效非負數 | 逐列檢查 | **硬擋** `wpls-bad-weights`，**指名列號** | `pls.js:2147–2160` |
| 權重總和 > 0 | 檢查 | **硬擋** | `pls.js:2164` |
| 加權後無零變異欄 | `weightedCorrMatrix` 回 `zeroVarIndex` | **硬擋** `zero-variance`，指名指標 | `pls.js:599`、`1452–1454` |
| 缺失機制為 MCAR | **不檢核** | 無警告（見第 6 節） | — |
| 權重的來源合理 | **不檢核** | 無警告 | — |

★ 權重為 0 的列會被計數並在警告中告知「$k$ 筆權重為 0 的資料列實質不參與估計」（`pls.js:2192`）。
...
| WPLS 警告（含權重 0 的列數與推論限制） | 3.5 | `pls.js:2192` |
| 缺失值處理選項（casewise／pairwise／mean） | §4 | `pls.js:508–530` |
| 抽樣權重欄位選擇 | 3.2 | `pls.js:2137–2166`（`resolveRowWeights`） |
...
**建議處置**：`intro` 句加入條件片語——
casewise 且有剔除時補「（listwise deletion 後剔除 $k$ 筆含缺失值之樣本）」；
pairwise 時補「（採 pairwise deletion，最少配對數 $m$）」；
有權重時補「並以抽樣權重加權估計（推論仍以未加權重抽建立）」。中英各一處。
**分級 L2**（呈現層，不動數值），但涉及新增 i18n 片語與句子組裝邏輯，列出供裁決。

### 待辦編號

本組開出 **R11（L1，已記錄）** 與 **R12（L2，待裁決）**。
