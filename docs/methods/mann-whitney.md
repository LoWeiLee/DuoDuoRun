...
| 4 | ★ **精確法缺口的窮盡量化（A5a 習慣 7、8）—— ★ 僅適用「無並列」** —— 無並列時精確 $p$ 只依賴 $(n_1,n_2,U)$，故可**對 $U$ 全枚舉**而非抽樣。以動態規劃建出 $U$ 的完整分布，掃描 $n_1\le n_2$、$3..25$ 的所有組合 × $U$ 全枚舉，共 **54,878 個格點**：<br>　• .05 判定翻面 **110 格（0.20%）**<br>　• ★ **全部 110 格都是「精確顯著、近似不顯著」的保守方向；危險方向＝ 0 格**<br>　• 最大絕對差 **0.0375**（$n_1=n_2=3$、$U=3$）<br>⇒ **在無並列的情形下**，缺 exact 法只會少抓到極少數真效果 |
| 5 | ★★ **有並列時方向相反（2026-07-30 R 抽驗開出，見 §8 的 R58）** —— 上一列的結論**不適用於有並列**。R 4.6 對有並列的情形用 Streitberg–Röhmel 位移演算法算**條件精確分布**（不是退回近似），本專案的 `ties` 基準組實測：<br>　• R 條件精確 $p$ = **0.022329**（沙盒獨立列舉 $\binom{24}{12}=2{,}704{,}156$ 種分組逐位重算，與 R 的 0.02233 一致）<br>　• 本工具常態近似（CC ＋ 並列校正）$p$ = **0.018117** ⇒ ★ **偏寬鬆**<br>　• 補掃 900 個有並列情境（每組 $n=5..9$ × 值域 2–4 個相異值）：**78.1% 是 anti-conservative**；★ **10 例（1.1%）在 .05 上偽顯著**，最糟一例近似 $p=0.0447$（報顯著）而精確 $p=0.1026$（不顯著，差 2.3 倍）<br>⇒ ★ **有並列 ＋ 小樣本時，本工具的 $p$ 可能偏小、報出 SPSS／R 不會給的顯著** |
...
2. ★ **從未與 SPSS 對照過**。「SPSS 的 Asymp. Sig. 不套用連續性校正」這句話的來源是**第三方教學文件，IBM 官方文件未取得** ⇒ 標為待核。
   ★ **R 那一側已於 2026-07-30 在 Kevin 本機實跑核實**（R 4.6.0），三個口徑全部對上：

   | 情境 | R 預設 | R 近似＋CC | R 近似不 CC | 本工具 |
   |---|---|---|---|---|
   | `main`（$n=30/30$，無並列） | **0.5229**（走精確法） | 0.5201 | 0.5154 | **0.5201** |
   | `small`（$n=4/4$） | **0.02857**（＝fixture 的 `pExact`） | 0.03038 | — | **0.030383** |
   | `ties`（$n=12/12$） | **0.02233**（條件精確，含並列） | — | — | **0.018117** ★ |

   ⇒ 「本工具＝R 的『近似＋CC』口徑」這句話**已證實**；而 R 的**預設**在三組裡都不是那個口徑
...
4. ★★ **`pExact` 是 SKIP 而非對照**：JS 無精確法，該欄永遠 `undefined`。★ **缺口的方向依有無並列而相反**——無並列時保守（上表第 4 道），**有並列時可能偽顯著**（第 5 道）。Kevin 2026-07-30 裁決：維持 backlog P2，但 UI 在「小樣本 ＋ 有並列」時加強警告（見 §8 的 R58）
...
### R58（L2，2026-07-30 R 抽驗開出）★ A5b 原本的「保守缺口」結論只在無並列時成立

**發現**　A5b 交付時我寫下「精確法缺口的危險方向為 0」，依據是 54,878 個格點的窮盡掃描。
★ **那個掃描的前提是「無並列」**——因為只有無並列時，精確分布才只依賴 $(n_1,n_2,U)$、才能對 $U$ 全枚舉。
2026-07-30 的 R 抽驗打到了這個盲點：R 4.6 對有並列的情形**不是**退回常態近似，
而是用 Streitberg–Röhmel 位移演算法算**條件精確分布**。

| | 本專案 `ties` 基準組（$n=12/12$、重度並列） |
|---|---|
| R 條件精確 $p$ | **0.022329** |
| 本工具常態近似（CC ＋ 並列校正） | **0.018117** |
| 方向 | ★ **近似偏寬鬆（anti-conservative）** |

★ 沙盒獨立列舉 $\binom{24}{12}=2{,}704{,}156$ 種分組重建同一條件分布，得 0.022329，**與 R 逐位一致**
——所以這不是 R 的實作特性，是條件精確分布本身。
...
★ **這是我在 A5b 的敘述錯誤，不是實作退步**：引擎的數字從頭到尾都是「常態近似 ＋ CC ＋ 並列校正」
這個方法的正確結果（與 scipy 逐值相符至 6e−14）。錯的是我把「無並列」的結論寫成了通則。

**處置（Kevin 2026-07-30 重新裁決：改文件 ＋ 小樣本含並列時加強警告）**

1. ✅ 本文件 §6 拆成兩道（無並列／有並列），§8 新增本條，`compare.test.js` 的
   `mann_whitney_small.pExact` SKIP 註解同步改寫
2. ✅ `docs/methods/README.md`、`roadmap-v2.md`、`validation-report-v1.md` 的 A5b 摘要全部更正
3. ✅ **UI 加強警告**：`smallSampleWarning` 觸發且 `tieCorrection` 為真時，
   多顯示一句明確說明「本工具報的是常態近似 $p$，有並列的小樣本下它可能**偏小**，
   SPSS 與 R 的精確法會給較大的 $p$」（i18n 新增 `smallSampleTiesNote`，zh／en 同步）
4. ⬜ 實作條件精確法仍維持 **backlog P2**（屬新功能，超出階段 A 範圍）
