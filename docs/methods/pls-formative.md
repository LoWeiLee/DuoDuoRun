...
→ `pls.js:1200–1243`（`coreEstimates`）。形成型構念的 loading 不用來判斷收斂效度，
...
→ `pls.js:1741–1747`。只對**形成型多指標**區塊計算（其餘回傳 `null`）。
...
→ `pls.js:1780–1796`（信效度）、`1796–1801`（Fornell-Larcker）、`942–968`（HTMT 的 `eligible` 過濾）

## 4. 假設前提與本工具的檢核方式

| 前提 | 本工具怎麼檢核 | 違反時的行為 | 位置 |
|---|---|---|---|
| 指標不完全共線（$\mathbf{S}_j$ 可逆） | 迭代前對每個 Mode B 區塊求反矩陣 | **估計中止** | `pls.js:661–672` |
| 指標不高度共線 | 外部 VIF | 燈號（不擋） | `pls.js:1741–1747` |
| 權重的統計顯著性 | bootstrap（見 `pls-bootstrap.md`） | 報表列 SE／t／p／CI | `pls.js:2393–2674` |
| 測量模式指定正確 | 不自動檢核 | 另由 CTA-PLS 以資料檢驗 | 見 `pls-cta.md` |
| 其餘（遞迴、樣本量、零變異） | 同 `pls-basic.md` §4 | | |

★ **完全共線時的實際行為（實測，紅隊 R7）**：Mode B 區塊的 $\mathbf{S}_j$ 不可逆時回傳
`estimation-failed`，訊息為「PLS 迭代過程出現數值退化（零變異 LV 分數或奇異矩陣），
請檢查指標間是否極度共線」（`pls.js:1451–1454`）。歸因方向正確，但**沒有指出是哪個構念、哪幾個指標**。
...
| UI 欄位 | 對應公式 | 程式碼 |
|---|---|---|
| 外部權重 Weight | 3.1 | `pls.js:1766–1779` 附近的 `outerWeights` 組裝 |
| 外部 VIF | 3.3 | `pls.js:1741–1747` |
| 外部負荷量（備援欄） | 3.2 | `pls.js:1200–1243` |
| 權重的 SE／t／p／CI | 見 `pls-bootstrap.md` | `pls.js:2393–2674` |
| α／rho_A／CR／AVE | 3.4（一律 `null`，UI 顯示「—」） | `pls.js:1780–1796` |
...
| 4 | 報表可追溯 | **通過** |
| 5 | 假設前提 | **通過**（另見 R7 的可改善項） |
| 6 | 慣例分歧 | **通過**（第 6 節列 3 項） |
| 7 | 邊界條件 | **發現 1 項**（R7，已實測）；另列 2 項未測（第 6 節第 2、5 點） |
...
### R7（L2，未修，待裁決）Mode B 區塊奇異時的錯誤訊息未指名構念
...
（`pls.js:1451–1454`），**不是** `not-converged`（後者只在 `ce.notConverged` 時觸發）。
歸因方向正確、訊息也點到共線，我第一次的判讀是錯的。

**真正剩下的缺口（弱化後）**：訊息**沒有指出是哪個構念、哪幾個指標**共線。
使用者在多構念模型裡拿到這句話，仍要自己一個個試。

**建議處置**：`estimateCoreFromCorr` 改為回傳結構化失敗原因（帶構念索引），
`estimateStage` 據以組出指名構念的訊息。**分級界於 L2／L3**——文字層是 L2，
但需要改動 `estimateCoreFromCorr` 的回傳契約（目前是 `object | null`），故列為**待 Kevin 裁決**。

### 待辦編號

本組開出 **R7（L2／L3，待裁決）**。跨組發現另見 `pls-plsc.md` 的 **R6（L4，已修）**。
