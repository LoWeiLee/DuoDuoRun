...
| | |
|---|---|
| 基準方法組數 | **82**（`tests/fixtures/reference.json`，每組附出處與容差理由） |
| 公式溯源已結案 | **80 / 82**（76 verified ＋ 4 純輸入型 exempt；2 組待原始文獻，見 `tests/fixtures/provenance.json`） |
| 回歸測試 | **1,184 過、6 記錄性跳過**（`npm test`；含統計層、i18n 對稱性、UI 行為） |
