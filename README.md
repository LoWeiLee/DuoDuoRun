...
| 回歸測試 | **1,237 過、6 記錄性跳過**（`npm test`；含統計層、i18n 對稱性、UI 行為） |
...
每一個可報告的統計方法都有一份獨立文件，說明**它在回答什麼問題、什麼時候不該用、公式是什麼、
本工具在有慣例分歧處採了哪一個、以及哪些部分還沒被驗證**——入口在
**[`docs/methods/`](docs/methods/README.md)**。

文件的第 3 節每一條公式都標註 `src/lib/stats/` 的行號區間，第 7 節把報表上每個數字對應回公式，
第 6 節則逐項列出**尚未驗證的部分**（例如某篇原始文獻尚未取得、某個慣例分歧未被基準覆蓋）。
這一節刻意不留白：目前 83 組基準中有 30 組屬 tier B（沒有可執行的第三方數值對照），
而歷來抓到的公式錯誤全部落在那一側，據實標示比宣稱「已完整驗證」有用。

> 建置中：PLS-SEM 的**測量與估計核心**（10 份）與**調節／高階構念／中介**（10 份）已完成，其餘方法陸續補齊。
...
| 2026-07 下旬 | **PLS-SEM 收尾**：品質殘項清空，並補上 PLSpredict 多次重複、MGA 的 PLSc 版、調節式中介（條件間接效果）。未取得原始文獻的術語與未核對的口徑一律在介面上明白標註，不含糊帶過 |
