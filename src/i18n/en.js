...
export default {
  ...
  helper: {
    fabTitle: 'Not sure which analysis?',
    fabHint: 'Duoduo helps you choose',
    fabBubble: 'Need help picking an analysis?',
    intro: "Hi, I'm Duoduo. Answer a few questions and I'll find the best analysis for you ✨",
    backBtn: 'Back',
    restartBtn: 'Start over',
    closeBtn: 'Close',
    recTitle: 'Recommended (click to jump to that analysis)',
    recNote: 'When multiple, ordered by best fit; pick based on your research context.',
    q: {
      start: 'What are you trying to do?',
      describe: 'How do you want to look at the data?',
      compareDv: 'What is the type of your dependent variable (DV)?',
      compareN: 'How many groups are you comparing?',
      compareTwoDesign: 'Independent groups or paired?',
      compareManyDesign: 'What is the design?',
      relate: 'What is your research question?',
      scale: 'What do you want to evaluate?',
      model: 'What kind of model do you want to test?',
      classify: 'Which type of analysis?',
    },
    opt: {
      describe: 'Describe the data (distribution, mean, SD, etc.)',
      compare: 'Compare differences across groups',
      relate: 'Examine relationships between variables',
      model: 'Test a multi-construct causal model (paths, mediation, necessity)',
      scale: 'Assess scale / inter-rater reliability',
      classify: 'Classification, dimensionality reduction, clustering',
      summary: 'Get N, mean, SD, and other basic stats',
      visualize: 'Scatterplot, histogram, boxplot',
      normality: 'Check whether data are normally distributed',
      continuous: 'Continuous (e.g., scores, age, salary)',
      categorical: 'Categorical (e.g., gender, preference, pass/fail)',
      nonnormal: 'Continuous but heavily skewed / small N',
      oneVsFixed: 'Compare to a fixed value (e.g., is mean = 70?)',
      twoGroups: '2 groups',
      threePlus: '3 or more groups',
      independent: 'Two different sets of people',
      paired: 'Same people in two situations (e.g., pre vs. post)',
      between: 'Different groups, one measure each (between-subjects)',
      twoFactor: 'Two categorical factors; main effects and interaction',
      within: 'Same people, multiple measures (within-subjects)',
      mixed: 'One between-subjects factor × one within-subjects factor',
      covariate: 'Compare groups while controlling another continuous variable',
      multiDv: 'Multiple DVs at once',
      corr: 'Just the strength of association between two variables',
      simpleReg: 'One X predicts a continuous Y',
      multiReg: 'Multiple Xs predict a continuous Y',
      logisticReg: 'Multiple Xs predict a binary Y (yes/no, pass/fail)',
      hierReg: 'Enter Xs in theory-driven blocks; check incremental contribution',
      reliability: 'Internal consistency of a scale (α)',
      explore: "Explore how many factors lie behind the data",
      confirm: 'Verify a pre-specified factor structure',
      agreementCat: 'Two raters agreeing on categorical judgments',
      agreementCont: 'Multiple raters agreeing on continuous scores',
      lda: 'Known classes; find linear combinations that separate them (with classification)',
      cluster: 'Unknown classes; group samples automatically',
      modelPls: 'Path relationships among latent constructs (PLS-SEM; incl. mediation, moderation)',
      modelCfa: 'First verify the measurement model (factor structure)',
      modelNca: 'Test a necessary condition ("no X, no Y") (NCA)',
    },
  },
  toolbar: {
    menuAria: 'Open navigation menu',
    toolsAria: 'Open tools menu',
  ...
  panels: {
    ...
    copyTablesBtn: 'Copy tables',
    copyTablesHint: 'Copy all result tables as tab-separated text; paste into Excel / Google Sheets to auto-split columns',
    copiedTables: 'Copied {n} table(s)',
    copyEmpty: 'No tables to copy yet',
    assumpTitle: 'Assumption check',
    assumpHint: 'Quick health check on whether the data fits this analysis — click to expand',
    assumpExpand: 'Expand',
    assumpCollapse: 'Collapse',
    assumpStatus: { ok: 'OK', warn: 'Caution', fail: 'Violated', info: 'Info', skip: 'Skipped' },
    assumpEmpty: 'No automatic assumption check for this analysis',
    assumpChecks: {
      sampleSize: 'Effective sample size',
      sampleSizeDetail: 'N = {n} (after listwise deletion)',
      sampleSizeDetailGroups: 'N = {n} ({k} groups, listwise)',
      sampleSizeDetailRatio: 'N = {n}; predictors p = {p} (rule of thumb: N ≥ {recommended})',
      normality: 'Normality (Shapiro-Wilk)',
      normalityGroup: 'Normality — group {group}',
      normalityVar: 'Normality — {var}',
      normalityDiff: 'Normality of difference (pre − post)',
      normalityDetail: 'n = {n}, W = {w}, p = {p}',
      varianceHomogeneity: 'Homogeneity of variance (Levene)',
      leveneDetail: 'F({df1}, {df2}) = {f}, p = {p}',
      tooFewForNormality: 'Too few cases (< 3) to test',
      tooFewForLevene: 'Not enough groups or cases',
    },
  ...
  lda: {
    ...
    result: {
      ...
      structureHint: 'Pooled within-group correlation between predictors and discriminant scores (SPSS / R MASS definition). |r| >= 0.30 is meaningful. Preferred for naming the construct.',
  ...
  datasets: {
    ...
    tpb: 'TPB civil servants AI survey',
    ...
    tpbDesc: 'TPB 5 constructs, 17 items + grade/seniority (N=200) — for PLS-SEM, CFA, \u03b1',
  },
  common: {
    comingSoon: 'Coming soon',
  ...
  errors: {
    stats: {
      'no-data': 'No usable rows (they may all be missing values).',
      'no-variation': 'The selected variable has no variation — every value is identical.',
      'zero-total-variance': 'Total variance is zero; there is nothing to decompose.',
      'x-is-constant': 'The predictor is constant, so its relationship with the outcome cannot be estimated.',
      'y-all-same-class': 'The outcome has only one class; a classification model cannot be fitted.',
      'y-not-binary': 'This method requires a binary (0/1) outcome.',
      'y-in-x': 'The outcome cannot also be selected as a predictor.',
      ...
      'singular-final-cov': 'The covariance matrix is singular (variables may be perfectly collinear).',
      'singular-reduced-model': 'The reduced model design matrix is singular — usually an empty factor cell or collinear predictors.',
      'unknown-type': 'Unknown analysis type.',
    },
    boundaryTitle: 'Something went wrong in this panel',
    boundaryBody: 'This analysis hit an unexpected error while rendering. Your data is safe and the other analyses still work — try again, or pick a different analysis from the sidebar.',
    boundaryWhere: 'Location',
    boundaryRetry: 'Try again',
    boundaryShowDetails: 'Show technical details',
    boundaryHideDetails: 'Hide technical details',
    boundaryHint: 'If it keeps failing, this variable combination is probably outside what the analysis supports (too few groups, zero variance, and so on). Picking a different set of variables usually works around it.',
  ...
  pls: {
    ...
    config: {
      ariaIntA: 'Interaction {n}: first construct',
      ariaIntB: 'Interaction {n}: second construct',
      ariaIntTarget: 'Interaction {n}: moderated endogenous construct',
      ariaPathFrom: 'Path {n}: source construct',
      ariaPathTo: 'Path {n}: target construct',
      ariaMgaGroupCol: 'Multigroup analysis: grouping column',
      ariaMgaGroupValue: 'Multigroup analysis: value for group {n}',
      ariaIpmaTarget: 'IPMA target construct',
      ariaIpmaScaleMin: 'IPMA theoretical scale minimum',
      ariaIpmaScaleMax: 'IPMA theoretical scale maximum',
      ...
      modeNote: 'W4 release: moderation (two-stage aligned with SmartPLS 4; product indicator / orthogonalizing optional), quadratic effects, three-way interactions, higher-order constructs (repeated indicators / two-stage / disjoint), and mediation decomposition with bootstrap CIs. From W3: Mode A/B, three schemes, PLSc, percentile/BCa, model fit, Q². Limits: PLSc and Q² are unavailable for models with interactions / higher-order constructs. Missing data handled by casewise deletion.',
      viewForm: 'Form',
      viewCanvas: 'Canvas',
      viewFormHint: 'Declare the model with dropdowns and multi-select; best for precise edits and mobile.',
      viewCanvasHint: 'The main area switches to a drag-and-drop model canvas that shares the same model as the form.',
      canvasNarrowFallback: 'The canvas needs a wider screen; on mobile it falls back to the form automatically.',
      interactionsTitle: 'Moderation & quadratic effects (interaction terms)',
      interactionsHint: 'Declare an A × B interaction, then point it at a dependent construct in the structural paths. Pick the same construct twice for a quadratic effect (two-stage only). Missing main-effect paths (A → Y, B → Y) are added automatically (SmartPLS 4 behavior).',
      addInteraction: 'Add interaction',
      removeInteraction: 'Remove this interaction',
      intFactor1: 'Construct A',
      intFactor2: 'Construct B',
      ...
      addHoc: 'Add higher-order construct',
      removeHoc: 'Remove this higher-order construct',
      hocNameLabel: 'Name',
      hocComponentsLabel: 'Lower-order constructs (≥ 2)',
      hocMethodTitle: 'Estimation approach (shared by all HOCs)',
      ...
      w5KTitle: 'k (folds)',
      w5RepsTitle: 'Repetitions',
      w5RepsHint: 'A single k-fold run varies with how the data happened to be split. Repeating and aggregating reduces that split noise; SmartPLS defaults to 10. Default here is 1 (single run); raising it lengthens computation. Aggregation: indicator metrics are averaged across repetitions, while for CVPAT the case-level losses are averaged first and tested once. Note: seminr uses the other convention (average the predictions across repetitions, then compute the metrics once), which systematically understates error \u2014 the gap equals the between-repetition variance of the predictions, an ensembling effect rather than out-of-sample performance. This tool does not follow it.',
      ...
      w5IpmaScaleLabel: 'Rescale on theoretical scale bounds (not observed min/max)',
      w5IpmaScaleHint: 'Denominator of the 0\u2013100 rescaling. The default uses observed min/max, which makes performance sensitive to sample extremes and not comparable across samples; entering the scale\u2019s own theoretical bounds (e.g. 1 and 7 for a 1\u20137 Likert item) matches SmartPLS and Ringle & Sarstedt (2016). Note that switching bounds changes both performance and importance (importance is an unstandardized effect on the 0\u2013100 score scale), so the two settings produce two different maps and must not be read together. Assumes every indicator shares the same scale.',
      w5IpmaScaleMin: 'Min (e.g. 1)',
      w5IpmaScaleMax: 'Max (e.g. 7)',
      w5NeedScale: 'Theoretical scale bounds need two numbers, and the maximum must exceed the minimum',
      ...
      w5PosHint: 'Complementary to FIMIX: it looks for the same unobserved segments but maximises the **sum of segment R²** instead of a likelihood, uses hard assignment plus hill-climbing, and assumes no distribution (Becker et al., 2013). When both methods agree, the segmentation is more credible.',
      w5PosKNote: 'PLS-POS shares the segment count K above with FIMIX. Note: the POS objective necessarily rises as K grows (it has no penalty term), so POS cannot be used to choose K — read the FIMIX information criteria and your theory for that.',
    ...
    canvas: {
      ...
      w4Hint: 'Interactions and higher-order constructs are configured in the form on the left; the canvas only displays them (drag to lay out, but you cannot add or rename them here).',
      legendW4: 'Amber dashed ellipse = higher-order construct (dashed links to its components); cocoa dashed box = interaction term (dashed links from its factor constructs); dashed arrow = main-effect path added automatically by the engine (hierarchical completeness for two-stage moderation).',
      autoPathMark: '(auto-added)',
      absorbedBadge: 'HOC component',
      hocMethodBadge: { disjoint: 'disjoint', 'two-stage': 'two-stage', repeated: 'repeated' },
      intMethodBadge: { 'two-stage': 'two-stage', 'product-indicator': 'product', orthogonal: 'orthogonal' },
    },
    result: {
      ...
      itcTitle: 'Structural model — model selection (IT criteria)',
      itcNote: '★ These four criteria only compare **competing predictor sets for the same endogenous construct**: they are meaningful when you hold two or more rival models whose dependent construct is identical, and smaller is better. They are **not comparable across constructs** (each row has a different dependent variable, so the absolute values carry no common scale) and **not comparable across datasets**. SSE = (n−1)(1−R²) (construct scores are standardized, so n−1 is the total variance); k is the number of antecedent constructs and the parameter count is k+1 (the error variance included). AIC and BIC follow the same form as seminr\'s compute_metrics.R; AICc is the small-sample correction (Hurvich & Tsai, 1989) and HQ is Hannan-Quinn — neither has a mainstream third-party PLS implementation, both being standard extensions of the same family. Only endogenous constructs appear here; nothing is computed when n − k − 2 ≤ 0 or SSE ≈ 0.',
      ...
      fitNote: 'SRMR: < .08 good (green), .08 – .10 borderline (yellow), ≥ .10 poor (red); NFI ≥ .90 good. d_ULS / d_G are distance measures for model comparison (smaller is better). ★ The log base of d_G (geodesic distance): this tool uses the **natural log** (the geodesic distance on the manifold of positive-definite matrices is defined as ½·Σ(ln λ)²); cSEM 0.6.1 uses base-10 logs, so the two differ by a factor of (ln10)² = 5.3019 — cSEM\'s own source carries a note saying they are unsure which base is correct. The decision rule is a bootstrap quantile (a relative comparison), so **the statistical conclusion is unaffected**, but the reported number differs from cSEM by 5.3×. "Saturated" frees all construct correlations (measurement model only); "estimated" implies them from the structural paths. Computed from the original formulas (SmartPLS does not fully document its implementation) — cross-check with SmartPLS / seminr recommended.\nGoF (Tenenhaus et al., 2005) = sqrt(mean communality × mean R²): officially NOT recommended as a fit index, provided only for comparison with older literature; communality from reflective multi-item constructs.',
      q2Title: 'Predictive relevance — blindfolding Q²',
      q2LegacyNote: 'Legacy metric: SmartPLS 4 removed blindfolding, and Hair et al. now recommend PLSpredict (out-of-sample prediction) and CVPAT instead \u2014 both are built into this tool (see the W5 settings above). Q\u00b2 is still reported for compatibility with the existing literature and older reviewer expectations; for submission, lead with PLSpredict / CVPAT and treat Q\u00b2 as supporting evidence.',
      ...
      stage1Note: 'Multi-stage model: the measurement statistics below (loadings, reliability, discriminant validity, fit) come from stage one (original indicators); the structural model (paths, R², f²) comes from the final stage (LV scores). Model fit is not reported for the final score-level model.',
      autoAddedLine: 'Main-effect paths added automatically: {paths} (SmartPLS 4 behavior)',
      interactionTitle: 'Moderation — interaction effects & simple slopes',
      interactionMethodLine: 'interaction generation = {method}',
      sdProductTag: ' | interaction term unstandardized (÷ SD of product)',
      quadraticTag: 'quadratic effect',
      slopesNote: 'Simple slopes (Aiken & West, 1991): conditional slopes at −1 SD / mean / +1 SD of the moderator; SE / t / p / CI are derived from the bootstrap path draws. Two-stage interaction coefficients are reported on the unstandardized-product scale (SmartPLS 4 convention); quadratic effects show tangent slopes at x = −1 / 0 / +1 SD. No simple slopes for product indicator / orthogonalizing (coefficients are on the standardized construct scale).',
      slopeLevelNames: { lo: '−1 SD', mid: 'Mean', hi: '+1 SD' },
      slopeCol: 'Conditional slope',
      slopeAxisX: '{iv} (SD)',
      slopeLegendModerator: 'Moderator {moderator}',
      mediationTitle: 'Mediation — effect decomposition (bootstrap CI)',
      mediationNote: 'A specific indirect effect whose bootstrap CI excludes 0 supports that mediating path (Preacher & Hayes, 2008). VAF = total indirect ÷ total effect (Hair et al., 2017): > 80% near-full mediation, 20% – 80% partial, < 20% limited (meaningful only when direct and indirect effects share the sign). Type classification (Zhao et al., 2010) appears in the report view.',
      effectCol: 'Effect',
      directLabel: 'Direct effect',
      indirectVia: 'Specific indirect: via {via}',
      totalIndirectLabel: 'Total indirect',
      totalLabel: 'Total effect',
      vafCol: 'VAF',
      interactionTag: ' (interaction)',
      vafNaLabel: 'VAF cannot be computed (total effect is near zero)',
      vafNoDirect: 'This pair has no direct path, so VAF is necessarily 100% — a consequence of the model specification, not an empirical finding; hidden here',
      vafOpposite: 'The direct and indirect effects have opposite signs, so VAF falls outside 0–100% and the 20%/80% benchmarks do not apply; hidden here',
      ...
      mgaNote: 'Interpret primarily via the permutation test (Chin & Dibbern, 2010; distribution-free). Henseler MGA p is the one-tailed P(β₁ ≤ β₂) — values near 0 or 1 both signal a group difference; the bias correction is anchored on the **bootstrap mean θ̄*** (the original formula in Henseler et al., 2009, matching seminr\'s estimate_pls_mga), not on the point estimate. Parametric tests (pooled / Welch) are shown for reference — the Welch test follows Sarstedt, Henseler & Ringle (2011) and weights the group variances by (n−1)/n. With equal group sizes the **t statistics are bit-for-bit identical, but the degrees of freedom and p values are not** (demo data, n = 30/30: df 58 vs 52.23, p .0194 vs .0198); with unequal sizes even the t values differ. Establish compositional invariance (MICOM) before MGA, otherwise differences may reflect measurement rather than structure.',
      mgaColG1: 'β ({g})',
      mgaColG2: 'β ({g})',
      mgaColDiff: 'Δβ',
      mgaColHenseler: 'p (Henseler)',
      mgaColHenseler2: 'p (Henseler, two-tailed)',
      ...
      posMeta: 'n = {n} | {p} hill-climbing passes, {m} moves | {st} random starts (highest objective kept) | minimum segment size {ms}',
      posNote: 'Algorithm: the objective is the sum of the endogenous constructs\' explained variance (R²) across segments. Each case is tentatively moved to every other segment and only improving moves are accepted, until a full pass makes no move (Becker, Rai, Ringle & Völckner, 2013, MISQ 37(3): "maximize the sum of the endogenous latent variables\' explained variance (R²) across all groups"). Hill-climbing only guarantees a local optimum, so several random starts are run and the best kept; segments are ordered by decreasing size to remove label switching. How it differs from FIMIX: POS uses hard assignment, an explained-variance objective, and no distributional assumption; FIMIX uses soft assignment (posterior probabilities), a mixture likelihood, and assumes normality. When the two methods find similar segments, the segmentation is more credible. ★ The key boundary: the POS objective necessarily rises as the number of segments grows — it has no penalty term, so it **cannot be used to choose K**. Use the FIMIX information criteria, theory, or the interpretability of the segments for that. ⚠ Scope of this implementation: a **structural-model-level simplification** of the original — latent variable scores come from the full-sample PLS weights and segment-specific measurement weights are not re-estimated (one of the paper\'s distinctive features), so heterogeneity confined to the measurement model will not be detected; the minimum segment size is this tool\'s stability constraint and is not part of the original method.',
      ...
      micomNote: 'Step 1 (configural): identical model, indicators and treatment across groups — satisfied by construction here. Step 2 (compositional invariance): pass when c ≥ the 5% permutation quantile (green); the accompanying p value is the permutation estimate of P(c* ≤ c_obs), computed as (#{c* ≤ c}+1)/(P+1) — a large p means the observed c is no lower than under random regrouping, which is the same criterion expressed the other way round. Step 3: the mean difference and the log variance ratio log(var₁)−log(var₂) (the original form in Henseler, Ringle & Sarstedt, 2016 — not a raw variance difference) inside the permutation 95% CI = full invariance; step 2 only = partial invariance (MGA is fine; comparing latent means is not). Note: c in step 2 is computed from composite scores on standardized indicators; cSEM 0.6.1 uses unstandardized data here, so its value differs slightly.',
      micomMeta: 'Groups: {g1} (n = {n1}) vs {g2} (n = {n2}) | {np} valid permutations',
      micomColC: 'c (step 2)',
      micomColP: 'p (permutation)',
      micomColQ5: '5% quantile',
      micomColMean: 'Mean diff [95% CI]',
      micomColVar: 'log variance ratio [95% CI]',
      modmedTitle: 'Moderated mediation \u2014 conditional indirect effects',
      modmedChain: '{x} \u2192 {m} \u2192 {y} (moderator: {w})',
      modmedOnA: 'first stage moderated (a path)',
      modmedOnB: 'second stage moderated (b path)',
      modmedBoth: 'both stages moderated',
      modmedColLevel: 'Moderator level',
      modmedColA: 'a (conditional)',
      modmedColB: 'b (conditional)',
      modmedColIndirect: 'Conditional indirect a\u00b7b',
      modmedAtMean: 'Mean (0)',
      modmedAtSd: '{s} SD',
      modmedSlope: 'Slope over the moderator',
      modmedLabelCaveat: '\u2605 On naming: the quantity labelled \u201cslope over the moderator\u201d is known in the literature as the index of moderated mediation, from Hayes (2015), Multivariate Behavioral Research 50(1). That original paper has NOT been obtained for this tool, so the quantity is reported under a descriptive name rather than that term, and no claim is made that the paper\u2019s specific test procedure is implemented (the CI here follows whichever bootstrap CI type you selected in advanced options). Check the original definition before using the term in a manuscript.',
      modmedNote: 'The conditional indirect effect is the conditional a path times the conditional b path. Construct scores are standardised, so the moderator is evaluated at \u22131 SD, the mean, and +1 SD (Aiken & West, 1991 \u2014 the same levels as the simple slopes above). Only when exactly one stage is moderated is the conditional indirect effect linear in the moderator, so only then is a single slope reported; when both stages are moderated the relationship is quadratic and no slope is shown. An interaction term has no built-in direction, so the same term yields both an \u201cX moderated by W\u201d and a \u201cW moderated by X\u201d reading \u2014 report the one your theory specifies, not both. Scope: two-step chains only, and only two-stage interactions with exactly two distinct factors.',
      mgaPlscTag: 'PLSc (consistent PLS)',
      predictReps: 'Aggregated over {r} repetitions of k-fold: indicator metrics are averaged; for CVPAT the case-level losses are averaged first and tested once',
      predictTitle: 'Out-of-sample prediction — PLSpredict (k = {k})',
      predictNote: 'Interpretation (Shmueli et al., 2019): require Q²predict > 0 first; then compare PLS vs LM RMSE — all lower = high predictive power, majority lower = medium, minority = low, none = lacking. ★ The paper does not fix a numeric threshold for \'majority\' vs \'minority\'; this tool reads it as \'more than half\', which is our own convention rather than a citation. The four-level verdict also presupposes Q²predict > 0, so it should not be quoted on its own when Q² fails. The LM Q²predict and MAE are listed alongside for reference (RMSE remains the basis of the verdict). CVPAT (Liengaard et al., 2021): d̄ > 0 with p < .05 means the PLS average loss is significantly below the benchmark.',
      predictColQ2p: 'Q²predict (PLS)',
      predictColQ2pLm: 'Q²predict (LM)',
      predictColRmsePls: 'RMSE (PLS)',
      predictColRmseLm: 'RMSE (LM)',
      predictColMae: 'MAE (PLS)',
      predictColMaeLm: 'MAE (LM)',
      predictVerdictLine: 'Overall verdict: {verdict} ({nBeatLm} of {nInd} endogenous indicators have a PLS RMSE below the LM benchmark; {nQ2ok} have Q²predict > 0)',
      predictVerdict: {
        high: 'high predictive power',
        medium: 'medium predictive power',
        low: 'low predictive power',
        none: 'lacking predictive power',
      },
      ...
      ipmaNote: 'Importance = unstandardized total effect on the target; performance = mean of 0–100 rescaled scores (Ringle & Sarstedt, 2016). The lower-right quadrant (important, low performance) is the priority area. Rescaling defaults to observed min/max; tick \u201cRescale on theoretical scale bounds\u201d in the W5 settings to use the scale\u2019s own bounds instead (the SmartPLS convention). The two differ whenever extremes are unobserved, and performance based on observed bounds is not comparable across samples. Target performance = {targetPerf}.',
      ipmaUpathTitle: 'IPMA — unstandardized path coefficients (0–100 score scale)',
      ipmaColUpath: 'Unstandardized coefficient',
      ipmaUpathNote: 'Importance is exactly what you get by multiplying these coefficients along each path and summing: the unstandardized total effect on the target construct. Each coefficient reads as "a one-point rise in the antecedent\'s 0–100 score raises the target\'s 0–100 score by this much on average" — a different scale from the standardized β in the structural model table, and the two are not comparable.',
...
FIMIX assumes the data come from a normal mixture and uses EM to maximise a likelihood, giving each case a probability of belonging to each segment. PLS-POS (Becker, Rai, Ringle & Völckner, 2013) assumes no distribution at all and maximises the sum of the segments' explained variance (R²): each case is hard-assigned to one segment, cases are tentatively moved between segments, and any move that raises that sum is accepted — until a full pass produces no move.
...
★ The key boundary: **POS cannot be used to choose the number of segments.** Its objective (the sum of segment R²) necessarily rises as K grows — more segments always fit better, because POS has no penalty term. Using POS to pick K yields the absurd conclusion that more is always better. Choose K from the FIMIX information criteria, from theory, or from how interpretable the segments are.

⚠ Scope of this implementation (disclosed): this is a **structural-model-level simplification** of the original method. Latent variable scores come from the full-sample PLS weights and segment-specific measurement weights are not re-estimated — yet detecting heterogeneity inside formative measurement models is one of the paper's distinctive features relative to FIMIX. Heterogeneity confined to the measurement model will therefore go undetected. The minimum segment size is a numerical-stability constraint added by this tool; the original has no such setting (it stresses the opposite — POS can surface very small niche segments).
...
· Finding a signal does not fix the estimate. The copula is a diagnostic; the remedy is at the design level — add the omitted variable, find an instrument, or use a method that handles simultaneity.`,
      ...
      w4:
        ...
        'Quadratic effects use the same mechanism with the squared score; three-way and higher interactions multiply the scores (include all lower-order terms to keep the specification hierarchical)\n' +
        'Product indicator / orthogonalizing build the interaction construct from indicator products (or their residuals); coefficients are on the standardized scale (aligned with seminr). Differences from two-stage are expected — pick one method and report it\n' +
        'Higher-order constructs: repeated indicators (HOC block = all LOC indicators, single estimation), disjoint two-stage (stage one links LOCs directly to the structure; recommended by recent literature) and embedded two-stage. Known repeated-indicators caveats: HTMT is undefined for HOC–LOC pairs (shown as —), model fit cannot be computed, and R² of HOC→LOC paths is mechanical rather than substantive\n' +
        'Mediation: decomposition into direct / specific indirect (products of path coefficients) / total indirect / total effects with bootstrap CIs (Preacher & Hayes, 2008); VAF and the Zhao et al. (2010) typology aid interpretation\n' +
        'Limits: PLSc and blindfolding Q² are unavailable for models with interactions / higher-order constructs; predictive assessment of such models arrives with PLSpredict (W5)',
    },
    apa: {
      intro:
      ...
      sigNo: 'not significant',
      r2: 'R² for {lv} = {r2}',
      moderation: 'For moderation, the interaction effect of {name} ({method}) on {to} was β = {beta}{stat}, {sig}',
      moderationSlopes: '; simple slopes showed conditional slopes of {iv} of {lo} and {hi} at −1 SD and +1 SD of {moderator}, respectively.',
      quadraticSentence: 'The quadratic effect of {name} on {to} was β = {beta}{stat}, {sig}.',
      modStat: ' (t = {t}, {pStr}, 95% CI [{cl}, {ch}])',
      modSigYes: 'statistically significant',
      modSigNo: 'not statistically significant',
      modMethodNames: { 'two-stage': 'two-stage, unstandardized interaction term', 'product-indicator': 'product indicator', orthogonal: 'orthogonalizing' },
      modmedIntro: 'Moderated mediation (conditional indirect effects, bootstrap 95% CI): ',
      modmedSentence: '{x} \u2192 {m} \u2192 {y} (moderator {w}): {levels}{slope}',
      modmedLevel: 'at {level} the indirect effect was {est}{ci} ({verdict})',
      modmedAtMean: 'the moderator\u2019s mean',
      modmedAtSd: '{s} SD of the moderator',
      modmedSig: 'CI excludes 0',
      modmedNs: 'CI includes 0',
      modmedSlopeClause: '; the slope of the indirect effect over the moderator was {est}{ci}',
      modmedBothClause: '; both stages are moderated, so the indirect effect is quadratic in the moderator and no single slope is reported',
      modmedTail: ' (Construct scores are standardised, so the moderator was evaluated at \u22121 SD, the mean, and +1 SD; Aiken & West, 1991. The slope above is known in the literature as the index of moderated mediation [Hayes, 2015]; that original paper was not obtained for this tool, so the quantity is reported under a descriptive name and the paper\u2019s specific test procedure is not implemented \u2014 verify the original definition before using the term.)',
      mediationIntro: 'Mediation decomposition (bootstrap 95% CI): ',
      mediationSentence: '{from} → {to}: total effect = {total}, direct effect = {direct}, total indirect = {indirect}{ci}{vaf}{type}',
      mediationCi: ' (CI [{lo}, {hi}])',
      mediationVaf: ', VAF = {vaf}',
      mediationType: ', classified as {type} (Zhao et al., 2010)',
      medTypes: {
        complementary: 'complementary (partial) mediation',
        competitive: 'competitive (partial) mediation',
        indirectOnly: 'indirect-only (full) mediation',
        directOnly: 'direct-only (no mediation)',
        none: 'no effect',
      },
      noDirect: 'none (no direct path specified)',
      // ── W5/W6 narrative sentences (added 2026-07-25, P1) ──────────────
      // Principle: each sentence only restates what the tables already report;
      // no new statistical claims. Every method boundary (Bonferroni, the
      // non-normality prerequisite, the EN threshold, POS not being usable for
      // choosing K, NCA's d >= .10 and p < .05) must appear in the sentence so
      // that users cannot over-claim after pasting it into a manuscript.
      micomIntro: 'Measurement invariance was assessed with the MICOM procedure (Henseler, Ringle & Sarstedt, 2016) for {g1} (n = {n1}) and {g2} (n = {n2}), using {np} valid permutations. Step 1 (configural invariance) is satisfied by design: both groups share the same model specification, indicators and data treatment. Results by construct: ',
      micomConstruct: '{lv} (c = {c}, 5% quantile = {q5}, permutation {cP}, {step2}; mean difference = {mDiff}, 95% CI [{mLo}, {mHi}]; log variance ratio = {vDiff}, 95% CI [{vLo}, {vHi}], {step3})',
      micomStep2Yes: 'step 2 established',
      micomStep2No: 'step 2 not established',
      micomStep3Yes: 'step 3 equal means and variances both established',
      micomStep3No: 'step 3 not fully established',
      micomFull: 'full measurement invariance holds for every construct, so both path coefficients and latent means may be compared across groups',
      micomPartial: 'partial measurement invariance holds — compositional invariance is established, so MGA path comparisons are admissible, but latent means must **not** be compared directly',
      micomNone: 'at least one construct fails compositional invariance (step 2), so **cross-group path comparisons are not meaningful** and any difference may reflect measurement rather than structure',
      micomTail: '. Overall verdict: {micomVerdict}.',
      mgaIntro: 'For the multigroup analysis (MGA), path coefficients were compared between {g1} (n = {n1}) and {g2} (n = {n2}) using a permutation test ({np} valid permutations): ',
      mgaPath: '{from} → {to} (β_{g1} = {b1}, β_{g2} = {b2}, difference = {diff}, permutation {pStr}, {sig})',
      mgaSigYes: 'a significant group difference',
      mgaSigNo: 'no significant group difference',
      mgaTail: '. Group comparisons are only meaningful once measurement invariance has been established; MICOM results should be inspected before interpreting this table.',
      predictIntro: 'For PLSpredict ({k}-fold cross-validation), {nQ2ok} of {nInd} indicators had Q²predict > 0 and {nBeatLm} had an RMSE below the linear-model (LM) benchmark, {predVerdict}',
      predVerdictSentence: {
        high: 'indicating high out-of-sample predictive power: the PLS model beats the linear benchmark on **every** endogenous indicator.',
        medium: 'indicating medium out-of-sample predictive power: the PLS model beats the linear benchmark on a **majority** (more than half) of the endogenous indicators.',
        low: 'indicating low out-of-sample predictive power: the PLS model beats the linear benchmark on only a **minority** (half or fewer) of the endogenous indicators.',
        none: 'indicating no out-of-sample predictive power: the PLS model beats the linear benchmark on **none** of the endogenous indicators.',
      },
      predVerdictCaveat: ' (Four-level criterion per Shmueli et al., 2019; the paper fixes no numeric threshold for "majority" versus "minority", so more than half is used here, and the verdict presupposes Q²predict > 0.)',
      predictCvpat: 'The CVPAT test gave an average loss difference of {dIA} against the indicator-average benchmark (t = {tIA}, {pIA}) and {dLM} against the linear-model benchmark (t = {tLM}, {pLM}).',
      ipmaIntro: 'The importance-performance map analysis (IPMA; Ringle & Sarstedt, 2016) used {target} as the target construct (performance = {targetPerf}): ',
      ipmaItem: '{lv} (importance = {imp}, performance = {perf})',
      ipmaTail: '. Importance is the standardised total effect and performance is the construct score rescaled to 0–100; constructs high in importance but low in performance are the priorities for improvement. This tool rescales using the observed minimum and maximum, so values differ from those obtained with the theoretical scale endpoints.',
      cipmaIntro: 'For the combined importance-performance map analysis (cIPMA; Hauff et al., 2024), the direct antecedents of {target} were examined with the CE-FDH ceiling line: ',
      cipmaItem: '{lv} (d = {d}, {pStr}, {verdict})',
      cipmaNecessaryYes: 'a necessary condition',
      cipmaNecessaryNo: 'not a necessary condition',
      cipmaTail: '. The criterion is an effect size d ≥ .10 together with a permutation p < .05 (Dul, 2016; Hauff et al., 2024); necessity additionally requires theoretical support, and being necessary does not imply being sufficient.',
      ctaIntro: 'Confirmatory tetrad analysis (CTA-PLS; Gudergan et al., 2008) tested the model-implied non-redundant tetrads using bias-corrected bootstrap confidence intervals with a within-block Bonferroni correction: ',
      ctaItem: '{lv} ({nTetrads} non-redundant tetrads, adjusted α = {alpha}, {verdict})',
      ctaVerdictFormative: 'at least one tetrad differed significantly from zero, disconfirming the reflective specification in favour of a formative one',
      ctaVerdictReflective: 'all tetrad confidence intervals covered zero, consistent with the reflective specification',
      ctaTail: '. CTA-PLS speaks to whether a reflective specification is compatible with the data; it does not by itself establish the content validity of a formative specification. Constructs with fewer than four indicators cannot form a tetrad and were excluded from the test.',
      copulaIntro: 'The Gaussian copula endogeneity check (Park & Gupta, 2012; procedure following Hult et al., 2018) assessed the significance of the copula terms using {b} bootstrap resamples: ',
      copulaSigYes: 'the copula term was significant in {n} model(s), suggesting possible endogeneity of the corresponding predictor construct(s)',
      copulaSigNo: 'no copula term reached significance, giving no evidence of endogeneity',
      copulaGateWarn: '. However, the construct scores for {lvs} could not be shown to be non-normal (KS test), whereas the identification condition of Park & Gupta (2012) requires non-normal predictors; the copula results therefore cannot be used to decide the endogeneity question here.',
      copulaGateOk: '. The prior KS tests indicated that all candidate construct scores were non-normal, satisfying the identification condition of Park & Gupta (2012).',
      fimixIntro: 'To examine unobserved heterogeneity, FIMIX-PLS (Hahn et al., 2002) with K = {k} (lnL = {lnl}, n = {n}) produced relative segment sizes of {shares}',
      fimixEn: ', with a normed entropy of EN = {en}, {enVerdict}',
      fimixEnOk: 'which meets the .50 threshold and indicates adequate separation between segments.',
      fimixEnBad: 'which falls below the .50 threshold, indicating inadequate separation; the segments should not be interpreted as distinct groups.',
      fimixNoEn: '. ',
      fimixTail: ' The number of segments should be decided jointly from the information criteria (AIC/BIC/CAIC and others; Sarstedt et al., 2011), theory, and the interpretability of the segments. If unobserved heterogeneity is present, the full-sample coefficients are a weighted average across segments and should not be generalised to individual subgroups.',
      posIntro: 'Prediction-oriented segmentation (PLS-POS; Becker et al., 2013) with K = {k} produced segments of size {sizes}, raising the overall R² from {r2g} to {r2s}. ',
      posTail: 'The POS objective is the sum of the segments\' explained variance; it necessarily increases with the number of segments and carries no penalty term, so it **cannot be used to choose K** — that decision should rest on the FIMIX information criteria, theory, or segment interpretability. This implementation operates at the structural-model level (segment-specific measurement weights are not re-estimated) and therefore addresses heterogeneity in structural relationships only.',
      copyHint: 'Copy APA narrative to clipboard',
    },
  },
  nca: {
