...
export default {
  ...
  pls: {
    ...
    result: {
      ...
      mgaNote: 'Interpret primarily via the permutation test (Chin & Dibbern, 2010; distribution-free). Henseler MGA p is the one-tailed P(β₁ ≤ β₂) — values near 0 or 1 both signal a group difference; the bias correction is anchored on the **bootstrap mean θ̄*** (the original formula in Henseler et al., 2009, matching seminr\'s estimate_pls_mga), not on the point estimate. Parametric tests (pooled / Welch) are shown for reference — the Welch test follows Sarstedt, Henseler & Ringle (2011) and weights the group variances by (n−1)/n, so it coincides with the pooled t when the groups are of equal size. Establish compositional invariance (MICOM) before MGA, otherwise differences may reflect measurement rather than structure.',
      mgaColG1: 'β ({g})',
      mgaColG2: 'β ({g})',
      mgaColDiff: 'Δβ',
