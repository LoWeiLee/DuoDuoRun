...
export default {
  ...
  pls: {
    ...
    config: {
      ...
      w5PosHint: '與 FIMIX 互補：同樣找未觀察的段，但目標是最小化**預測誤差**而非最大化概似，且為硬指派＋爬山法、不假設常態（Becker et al., 2013）。兩者結果一致時，分段結論較可信。',
      w5PosKNote: 'PLS-POS 與 FIMIX 共用上方的段數 K。注意：POS 的目標函數必然隨段數增加而下降（沒有懲罰項），所以不能拿 POS 來選段數——段數請看 FIMIX 的資訊準則與理論。',
    ...
    result: {
      ...
      mgaNote: '判讀以 permutation 檢定為主（Chin & Dibbern, 2010；分布無母數假設），Henseler MGA p 為單尾 P(β₁ ≤ β₂)——接近 0 或 1 都代表組間差異；參數檢定（pooled／Welch）供對照，Welch 依 Sarstedt, Henseler & Ringle (2011) 以 (n−1)/n 加權變異數，兩組人數相等時與 pooled t 恆等。進行 MGA 前應先以 MICOM 確認 compositional invariance，否則差異可能來自測量而非結構。',
      mgaColG1: 'β（{g}）',
      mgaColG2: 'β（{g}）',
      mgaColDiff: 'Δβ',
      ...
      posMeta: 'n = {n}｜爬山 {p} 輪、{m} 次搬移｜多起始分割 {st} 次（取目標函數最小者）｜段別下限 {ms} 筆',
      posNote: '演算法：目標函數＝各段各內生構念的殘差平方和（預測誤差），逐案試著搬到其他段、只接受能降低目標的搬移，掃到沒有任何搬移為止（Becker, Rai, Ringle & Völckner, 2013）。爬山法只保證局部最優，故採多起始分割取最佳；段別依大小遞減排序以消除 label switching。與 FIMIX 的差別：POS 是硬指派、目標是預測誤差、不假設常態；FIMIX 是軟指派（後驗機率）、目標是混合概似、假設常態。兩法找到相近的段時，分段結論才比較可信。★ 最重要的界線：POS 的目標函數必然隨段數增加而下降——它自己沒有懲罰項，所以**不能用來選段數**。段數要靠 FIMIX 的資訊準則、理論、或段別的可解釋性決定。',
...
FIMIX 假設資料來自常態混合分布，用 EM 最大化概似，給你每個個案「屬於各段的機率」。PLS-POS（Becker, Rai, Ringle & Völckner, 2013）不假設任何分布，直接把目標定為「預測誤差最小」：每個個案硬性歸屬一段，逐案試著搬到別段，只要能降低整體的殘差平方和就搬，掃到沒有任何搬移為止。
...
★ 最重要的界線：**POS 不能用來選段數。** 它的目標函數（預測誤差）必然隨段數增加而下降——段數愈多、配適愈好，因為 POS 自己沒有懲罰項。用 POS 選段數會得到「段數愈多愈好」的荒謬結論。段數要靠 FIMIX 的資訊準則、理論、或段別的可解釋性決定。
